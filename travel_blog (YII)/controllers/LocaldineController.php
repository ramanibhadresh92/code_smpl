...
use frontend\models\LocaldineReview;
use frontend\models\Notification;

class LocaldineController extends Controller
{
    ...
    public function actionCreatelocaldine() {
        ...
        return $this->render('createlocaldine', array('checkuserauthclass' => $checkuserauthclass));
    }

    public function actionEditlocaldine() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            ...
            if($checkuserauthclass != 'checkuserauthclassg' && $checkuserauthclass != 'checkuserauthclassnv') {
                if(isset($_POST['id']) && $_POST['id'] != '') { 
                    $id = $_POST['id'];
                    return $this->render('editlocaldine', array('checkuserauthclass' => $checkuserauthclass, 'id' => $id));
                }
            }
        }
    }

    public function actionCreatelocaldinesave() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            $post = $_POST;
            $files = $_FILES;
            return Localdine::createlocaldine($post, $files, $user_id);
        }
    ...
    public function actionEditlocaldinesave() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            $post = $_POST;
            $files = $_FILES;
            return Localdine::editlocaldine($post, $files, $user_id);
        }
    ...
    { 
        ...
        if(isset($_POST['$id']) && $_POST['$id'] != '') {
            if(isset($_POST['$src']) && $_POST['$src'] != '') {
                $id = $_POST['$id'];
                $src = $_POST['$src'];
                $data = Localdine::find()->where([(string)'_id' => $id, 'user_id' => $user_id])->andWhere(['not','flagger', "yes"])->one();
    ...
    public function actionAddreview() {
        ...
        if(isset($userid) && $userid != '') {
            ...
            if($authstatus == 'checkuserauthclassg' || $authstatus == 'checkuserauthclassnv') {
                ...
                $lasting = LocaldineReview::find()->where([(string)'_id' => $last_insert_id])->one();
                if(!empty($lasting)) {
    ...
    public function actionUploadphotoslocaldinesave() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            $post = $_POST;
            $files = $_FILES;
            return Localdine::uploadphotoslocaldinesave($post, $files, $user_id);
        }
    ...
    public function actionUploadphotoslocaldine() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            ...
            if($checkuserauthclass != 'checkuserauthclassg' && $checkuserauthclass != 'checkuserauthclassnv') {
                if(isset($_POST['id']) && $_POST['id'] != '') { 
                    $id = $_POST['id'];
                    return $this->render('uploadphotoslocaldine', array('checkuserauthclass' => $checkuserauthclass, 'id' => $id));
                }
            }
        }
    }
    ...
    public function actionDelete()  
    {
        ...
        if(isset($_POST['nid']) && $_POST['nid']) {
            $id = $_POST['nid'];
            $post = Localdine::find()->where([(string)'_id' => $id, 'user_id' => $user_id])->andWhere(['not','flagger', "yes"])->one();
            if(!empty($post)) {
                $post->delete();
