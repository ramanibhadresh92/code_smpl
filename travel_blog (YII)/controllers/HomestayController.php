...
use frontend\models\HomestayReview;
...
class HomestayController extends Controller
{
    ...
    public function actionCreatehomestayhtml() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        return $this->render('createhomestay',array('checkuserauthclass' => $checkuserauthclass));
    }

    public function actionEdithomestayhtml() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        if(isset($user_id) && $user_id != '') {
            $checkuserauthclass = UserForm::isUserExistByUid($user_id);
            if($checkuserauthclass != 'checkuserauthclassg' && $checkuserauthclass != 'checkuserauthclassnv') {        
                if(isset($_POST['id']) && $_POST['id'] != '') {
                    $id = $_POST['id'];
                    return $this->render('edithomestay',array('checkuserauthclass' => $checkuserauthclass, 'id' => $id));
                }
            }
        }
    }

    public function actionCreatehomestay() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            $post = $_POST;
            $files = $_FILES;
            return Homestay::createhomestay($post, $files, $user_id);
        }
    ...
    public function actionEdithomestay() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            $post = $_POST;
            $files = $_FILES;
            return Homestay::edithomestay($post, $files, $user_id);
        }
    ...
    { 
        ...
        if(isset($_POST['$id']) && $_POST['$id'] != '') {
            if(isset($_POST['$src']) && $_POST['$src'] != '') {
                $id = $_POST['$id'];
                $src = $_POST['$src'];
                $data = Homestay::find()->where([(string)'_id' => $id, 'user_id' => $user_id])->andWhere(['not','flagger', "yes"])->one();
    ...
    public function actionAddreview() {
        ...
        if(isset($userid) && $userid != '') {
            ...
            if($authstatus == 'checkuserauthclassg' || $authstatus == 'checkuserauthclassnv') {
                ...
                $lasting = HomestayReview::find()->where([(string)'_id' => $last_insert_id])->one();
                if(!empty($lasting)) {
    ...
    public function actionUploadphotoshomestaysave() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            ...
            if($checkuserauthclass != 'checkuserauthclassg' && $checkuserauthclass != 'checkuserauthclassnv') {        
                if(isset($_POST) && !empty($_POST)) {
                    $post = $_POST;
                    $files = $_FILES; 
                    $result = Homestay::uploadphotoshomestaysave($post, $files, $user_id);
                    return $result;
                    exit;
                }
            }
    ...
    public function actionUploadphotoshomestay() {
        $session = Yii::$app->session;
        $user_id = (string)$session->get('user_id');
        ...
        if(isset($user_id) && $user_id != '') {
            ...
            if($checkuserauthclass != 'checkuserauthclassg' && $checkuserauthclass != 'checkuserauthclassnv') {
                if(isset($_POST['id']) && $_POST['id'] != '') { 
                    $id = $_POST['id'];
                    return $this->render('uploadphotoshomestay', array('checkuserauthclass' => $checkuserauthclass, 'id' => $id));
                }
            }
        }
    }
    ...
    public function actionDelete()  
    {
        ...
        if(isset($_POST['nid']) && $_POST['nid']) {
            $id = $_POST['nid'];
            $post = Homestay::find()->where([(string)'_id' => $id, 'user_id' => $user_id])->andWhere(['not','flagger', "yes"])->one();
            if(!empty($post)) {
                $post->delete();
