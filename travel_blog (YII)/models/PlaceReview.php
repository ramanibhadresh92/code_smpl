...
class PlaceReview extends ActiveRecord
{
    ...
    {
        return PlaceReview::find()->with('user')->where(['is_deleted'=>"0"])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->all();
    }
	
	public function getAllPosts()
    {
        return PlaceReview::find()->where(['is_deleted'=>"0"])->andWhere(['not','flagger', "yes"])->all();
    }
	
	public function getAllPhotoPosts()
    {
        return PlaceReview::find()->where(['is_deleted'=>"0",'post_type'=>'image'])->orwhere(['post_type'=>'text and image'])->andWhere(['not','flagger', "yes"])->all();
    }
    ...
    public function getUserPost($userid, $start='')
    {
    	...
    	{
            return PlaceReview::find()->with('user')->where(['post_user_id'=>"$userid",'is_deleted'=>'0','rating'=>null,'collection_id'=>null,'trav_item'=>null,'is_trip'=>null,'is_ad'=>null,'placetype'=>null])->orwhere(['like','post_tags',"$userid"])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->limit(7)->offset($start)->asarray()->all();
    	}
    	else
    	{
            return PlaceReview::find()->with('user')->where(['post_user_id'=>"$userid",'is_deleted'=>'0','rating'=>null,'collection_id'=>null,'trav_item'=>null,'is_trip'=>null,'is_ad'=>null,'placetype'=>null])->andWhere(['not in', 'post_privacy', 'Private'])->orwhere(['like','post_tags',"$userid"])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->limit(7)->offset($start)->asarray()->all(); 
        }
    }
    ...
    public function getUserPostUserwall($userid, $start='')
    {
        ...
        if($uid == (string)$userid)
        {
            return PlaceReview::find()->with('user')->where(['post_user_id'=>"$userid",'is_deleted'=>'0'])->orWhere(['shared_by' => $userid, 'shared_from' => 'shareentity'])->orwhere(['like','post_tags',"$userid"])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->limit(7)->offset($start)->asarray()->all();
            
        }
        else
        {
            return PlaceReview::find()->with('user')->where(['post_user_id'=>"$userid",'is_deleted'=>'0'])->andWhere(['not in', 'post_privacy', 'Private'])->orWhere(['shared_by' => $userid, 'shared_from' => 'shareentity'])->orwhere(['like','post_tags',"$userid"])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->limit(7)->offset($start)->asarray()->all(); 
        }
    }
	...
	public function getUserPostCount($userid)
    {
    	...
    	if($uid == (string)$userid)
    	{
            return PlaceReview::find()->with('user')->where(['post_user_id'=>"$userid",'is_deleted'=>'0','rating'=>null,'collection_id'=>null,'trav_item'=>null,'is_trip'=>null,'is_ad'=>null,'placetype'=>null])->orwhere(['like','post_tags',"$userid"])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->all();
    	}
    	else
    	{
            return PlaceReview::find()->with('user')->where(['post_user_id'=>"$userid",'is_deleted'=>'0','rating'=>null,'collection_id'=>null,'trav_item'=>null,'is_trip'=>null,'is_ad'=>null,'placetype'=>null])->andWhere(['not in', 'post_privacy', 'Private'])->orwhere(['like','post_tags',"$userid"])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->all(); 
        }
    }
    
	public function getTravPostCount()
    {
        return PlaceReview::find()->with('user')->where(['is_deleted'=>'0','rating'=>null,'collection_id'=>null,'trav_item'=>null,'is_trip'=>null,'is_ad'=>null,'placetype'=>null])->andWhere(['not in', 'post_privacy', 'Private'])->andWhere(['not','flagger', "yes"])->count();
    }
     ...
     public function getUserPhotos($userid)
    {
    	...
    	if($uid == (string)$userid)
    	{
            return PlaceReview::find()->with('user')->where(['post_user_id'=>"$userid",'is_deleted'=>'0','is_album'=>'1','is_timeline'=>null])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->all();
    	}
    	else
    	{
            return PlaceReview::find()->with('user')->where(['post_user_id'=>"$userid",'is_deleted'=>'0','is_album'=>'1','is_timeline'=>null])->andWhere(['not in', 'post_privacy', 'Private'])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->all(); 
        }
    }
     ...
     public function getUserNotifications()
    {
        ...
        $connect_posts =  PlaceReview::find()->with('user')->where(['in','post_user_id',$ids])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->all();
    ...
    public function getPlaceReviews($place,$type,$count)
    {
		...
		if($count == 'all') {
			return PlaceReview::find()->with('user')->where(['is_deleted'=>"0",'placetype'=>"$type"])->andwhere(['not in','_id',$hide_ids])->andWhere(['not','flagger', "yes"])->asarray()->orderBy(['post_created_date'=>SORT_DESC])->all();
		} else {
			return PlaceReview::find()->with('user')->where(['is_deleted'=>"0",'placetype'=>"$type"])->andwhere(['not in','_id',$hide_ids])->andWhere(['not','flagger', "yes"])->asarray()->orderBy(['post_created_date'=>SORT_DESC])->limit($count)->offset(0)->all();
		}
    }
    ...
    public function getPlaceReviewsCount($place,$type)
    {
		...
		return PlaceReview::find()->with('user')->where(['is_deleted'=>"0",'placetype'=>"$type"])->andwhere(['not in','_id',$hide_ids])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->count();
    }
	
    public function getPlacePost($type)
    {
        return PlaceReview::find()->with('user')->where(['is_deleted'=>"0",'placetype'=>"$type"])->andWhere(['not','flagger', "yes"])->orderBy(['post_created_date'=>SORT_DESC])->all();
    }

	public function DeletePlaceReviewCleanUp($post_id,$post_user_id)
    {
        ...
        $delete_post = new PlaceReview();
		$date = time();
        $data = array();
        $delete_post = PlaceReview::find()->where(['_id' => $post_id])->andWhere(['not','flagger', "yes"])->one();
		...
		if(isset($delete_post['parent_post_id']) && !empty($delete_post['parent_post_id']))
		{
			...
			$main_post = PlaceReview::find()->where(['_id' => $parent_post_id])->andWhere(['not','flagger', "yes"])->one();
		...
		/* Delete All Images Releted To This (Delete Post) */
			...
			if(isset($delete_post['image']) && $delete_post['image'] != '') {
