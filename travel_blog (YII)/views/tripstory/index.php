			                                ...
			                                <div class="row cshfsiput cshfsi">
												...
												<div class="postBox">
													<div class="content-box">
														<div class="new-post base-newpost cshfsiput cshfsi <?=$checkuserauthclass?>" id="composetoolboxAction">
...
{
	$this->context->getnolistfound('notripstoryfound');
}
$templp = 1;
$lp = 1; 
...
{ 
	...
	if($isOk == 'ok2389Ko') {
		if(($lp%8) == 0) {
			...
			if(isset($ads) && !empty($ads))
			{
				...
				$this->context->display_last_tripstory($ad_id, $existing_posts, '', $cls);
				$lp++; 
			} else {
				$lp++;
			}
		} else {
			$this->context->display_last_tripstory($postid, $existing_posts, '', $cls);
			$lp++;	
		}						
	}
	$templp++;
	...
	<input type="hidden" name="pagename" id="pagename" value="tripstory" />
	...
	<script src="<?=$baseUrl?>/js/trip_story.js"></script>
	<?php $this->endBody() ?> 