...
$collections = Collections::find()->where([(string)'_id' => $id])->andWhere(['not','flagger', "yes"])->one();
...
if(!empty($collections)) {
    ...
    $title = isset($collections['title']) ? $collections['title'] : ''; 
    $description = isset($collections['description']) ? $collections['description'] : ''; 
    $location = isset($collections['location']) ? $collections['location'] : ''; 
    $visible_to = isset($collections['visible_to']) ? $collections['visible_to'] : '';
    $tagged_connections = isset($collections['tagged_connections']) ? $collections['tagged_connections'] : ''; 
    ...
    $collectionsImg = $collections['image'];
    $collectionsImg = explode(',', $collectionsImg);
    $collectionsImg = array_values(array_filter($collectionsImg));
    ...
    <div class="modal_content_container" data-editid="<?=$id?>">
        <div class="modal_content_child modal-content">
            <div class="popup-title">
                ...
                <a type="button" class="item_done crop_done hidden_close_span custom_close waves-effect" href="javascript:void(0)" data-editid="<?=$id?>" onclick="tempedit_ps(this)">Done</a>
            </div>
                                                            ...
                                                            <div class="post-photos new_pic_add">
                                                                <div class="img-row">
                                                                    ...
                                                                    foreach ($collectionsImg as $collectionsImg_s){
                                                                        ...
                                                                        $uniqid = $unid.'_'.$rand;
                                                                        ?>
                                                                        <div class="img-box"><img src="<?=$collectionsImg_s?>" class="upldimg thumb-image vimg"><div class="loader" style="display: none;"></div><a href="javascript:void(0)" onclick="removepiccollections_modal('<?=$id?>', this)" class="removePhotoFilecollections"><i class="mdi mdi-close"></i></a></div>
                                                                    ...
                                                                    <div class="img-box">
                                                                        <div class="custom-file addimg-box add-photo ablum-add">
                                                                            ...
                                                                            <input class="upload custom-upload remove-custom-upload edit-collections-file-upload" id="edit-collections-file-upload" title="Choose a file to upload" data-class=".post-photos .img-row" type="file">
                                                                        </div>
                                                                    </div>
                                                                    <?php } ?>
                                                                </div>
    ...
    <div class="valign-wrapper additem_modal_footer modal-footer">
        ...
        <a href="javascript:void(0)" class="btngen-center-align waves-effect" data-editid="<?=$id?>" onclick="tempedit_ps(this)">Update</a>
    </div>
    <?php 
}
exit;
