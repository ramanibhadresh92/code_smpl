...
$PRIFIX = 'web/v1/';
Route::post($PRIFIX.'tradex-service-invoice-api','Modules\Web\Http\Controllers\ServiceController@TradexServiceInvoiceAPI')->name("TradexServiceInvoiceAPI");
Route::post($PRIFIX.'generate-tradex-service-invoice-api','Modules\Web\Http\Controllers\ServiceController@TradexServiceInvoiceGenerateAPI')->name("TradexServiceInvoiceAPI");
Route::get('send-service-invoice-email','Modules\Web\Http\Controllers\ServiceController@ServiceInvoiceEmailSend')->name('ServiceMailSend');
...
Route::post('getDepartmentList','Modules\Web\Http\Controllers\UnloadController@getDepartmentList')->name('getDepartmentList');
Route::post('client-registration','Modules\Web\Http\Controllers\SalesController@ClientRegistration')->name('client-registration');
Route::post('GetByReferenceNo','Modules\Web\Http\Controllers\DashboardController@GetByReferenceNo')->name('GetByReferenceNo');
Route::post('customer-registration','Modules\Web\Http\Controllers\CustomerController@CustomerRegistration')->name('customer-registration');
Route::post('salution-list','Modules\Web\Http\Controllers\CustomerController@salution')->name('company-salution-list');
 ######### COMMON API ###########
...
Route::post("getGSTStateCode","Modules\Web\Http\Controllers\SalesController@GetGSTStateCode")->name('GetGSTStateCode');
...
Route::group(['middleware' => ['web','localization','jwt.auth','cors']], function() use($PRIFIX)
{
	Route::group(['middleware' => ['checkToken'], 'prefix' => $PRIFIX, 'namespace' => 'Modules\Web\Http\Controllers'],function (){
		...
		Route::post('verify-token','UserManagementController@checkToken')->name('verify-token');
		...
		*/
		...
		{
			Route::post('generate-customer-service-invoice','CustomerController@generateCustomerServiceInvoice')->name('generateCustomerServiceInvoice');
	        ...
	        Route::post('generate-customer-invoice','CustomerController@generateCustomerInvoice')->name('generateCustomerInvoice');
	         Route::post('generate-customer-service-invoice','CustomerController@generateCustomerServiceInvoice')->name('generateCustomerServiceInvoice');
	        // Route::post('generate-customer-digital-invoice','CustomerController@generateCustomerDigitalSignatureInvoice')->name('generateCustomerDigitalSignatureInvoice');
	        Route::post('generate-customer-einvoice','CustomerController@generateCustomerEInvoice')->name('generateCustomerEInvoice');
	        Route::post("customer-slab-invoice-list","CustomerController@CustomerSlabwiseInvoiceDetailsList")->name('CustomerSlabwiseInvoiceDetailsList');
	        Route::post('generate-slab-einvoice','CustomerController@GenerateCustomerSlabEinvoice')->name('GenerateCustomerSlabEinvoice');
	        Route::post("save-customer-invoice","CustomerController@SaveCustomerInvoice")->name('SaveCustomerInvoice');
	        Route::post("customer-invoice-getbyid","CustomerController@CustomerInvoiceGetByID")->name('CustomerInvoiceGetByID');
	        Route::post('mail-send-customer','CustomerController@MailSendFunction')->name('MailSendFunction');
		...
		Route::group(['prefix' =>'reports'], function() {
			Route::post("vendor-ledger-balance-data","ReportsController@GetVendorLedgerBalanceData")->name('GetVendorLedgerBalanceData');
			Route::post("vendor-ledger-balance-report","ReportsController@VendorLedgerBalanceReport")->name('VendorLedgerBalanceReport');
		...
		{
			Route::post('iot-alarm-graph','IotDashboardController@IOTAlarmGraph')->name('IOTAlarmGraph');
			Route::post('iot-alarm-widget','IotDashboardController@IOTDashboardAlarmWidget')->name('IOTDashboardAlarmWidget');
			Route::post('iot-alarm-data-details','IotDashboardController@IOTAlarmDataDetails')->name('IOTAlarmDataDetails');
			...
			############ Production Vs Projection Vs Actual Dashboard Widget ##############
			Route::post('send-reference-link-by-mail','DashboardController@SendReferenceLink')->name('SendReferenceLink');
		});
		...
		Route::group(['prefix' =>'sales'], function()
		{
			...
			Route::post('update-dispatch-po','SalesController@UpdateDispatchPO')->name('UpdateDispatchPO');
			Route::post('dispatch-parent-list', function(){
				$data = App\Models\Parameter::where("para_parent_id",DISPATCH_TYPE_PARAMETER)
						->where("status","A")
						->get();
				return response()->json(['code' => SUCCESS , "msg"=>trans('message.RECORD_FOUND'),"data"=>$data]);
			...
			Route::post('dispatch-child-list', function(){
				$data = App\Models\Parameter::where("para_parent_id",DISPATCH_CHILD_TYPE_PARAMETER)
						->where("status","A")
						->get();
				return response()->json(['code' => SUCCESS , "msg"=>trans('message.RECORD_FOUND'),"data"=>$data]);
			...
			Route::post('collection-cycle-terms', function(){
				$data = App\Models\Parameter::where("para_parent_id",PARA_COLLECTION_CYCLE_TERMS)
						->where("status","A")
						->orderBy("para_sort_order","ASC")
						->get();
				return response()->json(['code' => SUCCESS , "msg"=>trans('message.RECORD_FOUND'),"data"=>$data]);
			...
			{
				Route::post('update-additional-credit-limit','SalesController@UpdateClientCreditLimit')->name('UpdateClientAdditionalCreditLimit');
				...
				Route::post('update-po-detail','ClientPurchaseOrdersController@UpdatePurchaseOrderDetail')->name('UpdatePurchaseOrderDetail');

				Route::post('ReferenceNumberList','DashboardController@ReferenceNumberList')->name('ReferenceNumberList');
				################## PURCHASE ORDER END ##############
			});
		...
		Route::group(['prefix' =>'invoice'], function(){
			...
			Route::post('creditnote-generate_reason-list','InvoiceCreditDebitNotesController@getCreditNoteReasons')->name('getCreditNoteReasons');
		});
		...
		Route::group(['prefix' =>'iotbreakdown'], function() {
			Route::post('list-breakdowns','IotDashboardController@getBreakdownDetailsList')->name('getBreakdownDetailsList');
			Route::post('list-devices','IotDashboardController@getBreakdownDevices')->name('getBreakdownDevicesList');
			Route::post('save-breakdown-details','IotDashboardController@saveBreakdownDetails')->name('saveBreakdownDetails');
			Route::post('close-breakdown-details','IotDashboardController@closeBreakdownDetails')->name('closeBreakdownDetails');
			Route::post('get-breakdown-details','IotDashboardController@getBreakdownDetails')->name('getBreakdownDetails');
			Route::post('list-device-breakdown-reasons','IotDashboardController@getBreakdownDeviceReasons')->name('getBreakdownDeviceReasonsList');
			Route::post('list-device-breakdown-reason-actions','IotDashboardController@getBreakdownDeviceReasonActions')->name('getBreakdownDeviceReasonActionsList');
		});
		################## IOT MAINTENANCE MODULE ##############
	});
});
