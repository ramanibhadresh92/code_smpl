...
use Input;
use DB;
class WebPortalController extends LRBaseController
{
	 /**
	 *
	...
	*/
	public function getImpactReportDetails(Request $request)
	{
		...
		if (!empty($this->report_starttime) && !empty($this->report_endtime)) {
			...
			$LocationIDS 						= !empty($this->location_id)?$this->location_id:array();
			$CCOF->TotalManpowerInformation($this->report_starttime,$this->report_endtime,$arrLocations,$LocationIDS,$this->company_id);
