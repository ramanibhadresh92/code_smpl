...
class UpdatePurchaseStockAvgPriceDumyTable extends Command
{
	/**
	 ...
	 * @var string
	 */
	protected $signature = 'UpdatePurchaseStockAvgPriceDumyTable';
	 ...
	 * @var string
	 */
	protected $description = 'UpdatePurchaseStockAvgPriceDumyTable';
	...
	public function handle()
	{
	    ...
	    $end            = new DateTime('2022-02-02');
	    $interval       = DateInterval::createFromDateString('1 day');
	    $period         = new DatePeriod($begin, $interval, $end);
	    $PRODUCT_ARR    = CompanyProductMaster::where("para_status_id",6001)->pluck("id")->toArray();
	    // $MRF_ID         = 3;
	    $PRODUCT_TYPE   = 1;
	    $MRF_IDS 		= array(3,11,22,23,26,27,48);
		foreach($MRF_IDS AS $MRF_ID){
		    foreach ($period as $dt) {
		        $STOCK_DATE  = $dt->format("Y-m-d");
		        foreach($PRODUCT_ARR AS $PRODUCT_ID){
		            $LEDGER_ENTRY   =   \DB::table("purchase_avg_data_update")
		            ...
		            if(!empty($LEDGER_ENTRY)){
		            	foreach($LEDGER_ENTRY AS $KEY => $ENTRY_VAL){
				            ...
				            $OPENING_AVG    = (isset($DATA->avg_price)) ? $DATA->avg_price : 0;
		                   	...
		                   	$TOTAL_IN_QTY   =  \DB::table("purchase_avg_data_update")
		                    ->where("product_id",$PRODUCT_ID)
		                    ->where("mrf_id",$MRF_ID)
		                    ->where("process",1)
		                    ->where("type",1)
		                    ...
		                    $TOTAL_OUT_QTY  =   \DB::table("purchase_avg_data_update")
		                    ->where("product_id",$PRODUCT_ID)
		                    ->where("process",1)
		                    ->where("product_type",PRODUCT_PURCHASE)
		                    ->where("type",2)
		                    ...
		                    \DB::table("purchase_avg_data_update")
