...
class GenerateMaterialRecoveryReport extends Command
{
	/**
	 ...
	 * @var string
	 */
	protected $signature = 'GenerateMaterialRecoveryReport';
			...
			foreach($arrCompany as $Company)
			{
				$SELECT_SQL 	= "	SELECT wm_department.department_name,
									...
									(
										...
										FROM stock_ladger
										WHERE (stock_ladger.product_type = 1 OR stock_ladger.product_type = 2)
										AND stock_ladger.mrf_id = wm_department.id
									...
									GetRecoveryPercentageV2(wm_department.id,'$TODAY','$TODAY',0) AS TODAY_RECOVERY_PERCENTAGE,
									CASE WHEN 1=1 THEN
									(
										SELECT SUM(quantity)
										FROM outward_ledger
									...
									GetSalesPercentageV2(wm_department.id,'$TODAY','$TODAY',0) AS TODAY_SALES_PERCENTAGE,
									CASE WHEN 1=1 THEN
									(
										SELECT SUM(quantity)
										FROM inward_ledger
									...
									GetRecoveryPercentageV2(wm_department.id,'$MONTH_START','$TODAY',1) AS MONTH_RECOVERY_PERCENTAGE,
									CASE WHEN 1=1 THEN
									(
										SELECT SUM(quantity)
										FROM outward_ledger
									...
									GetSalesPercentageV2(wm_department.id,'$MONTH_START','$TODAY',1) AS MONTH_SALES_PERCENTAGE
									FROM wm_department
					...
					$ToEmail		= "virendra.mane@nepra.co.in,samir.jani@nepra.co.in,ankit.shah@nepra.co.in";
					$BccEMail		= "kalpak@yugtia.com,d.patel@nepra.co.in";
					// $ToEmail		= "kalpak@yugtia.com,axay.shah@yugtia.com";
					...
					$sendEmail 		= Mail::send("email-template.materialrecovery",$MatrialRecoveryReport, function ($message) use ($ToEmail,$FromEmail,$Subject,$BccEMail) {
