...
class TestConsoleNetSuit extends Command
{
	/** @var OAuth */
	public $oauth;
	 ...
	 * @var string
	 */
	protected $signature = 'TestConsoleNetSuit';
	 ...
	 * @var string
	 */
	protected $description = 'Console To Test Netsuit Api';
	...
	public function handle()
	{
		...
		// $NetSuit 					= new NetSuit();
		// $arrPostFields 				= array("fromDate"=>"2022-06-24","toDate"=>"2022-06-24");
		...
		// $NetSuitResponse 	 		= $NetSuit->GetDataFromNetSuit($arrPostFields,353,1);
		// print_r($NetSuit->API_RESPONSE);
		// $NetSuitData 				= json_decode($NetSuitResponse);
		// print_r($NetSuitData,true);

		//$SELECT_SQL = "SELECT id FROM wm_dispatch WHERE approval_status = 1 AND client_master_id IN (889) ORDER BY id DESC";
		$SELECT_SQL = "SELECT id FROM wm_dispatch WHERE approval_status = 1 ORDER BY id ASC";
		...
		if (!empty($SELECT_RES)) {
			foreach ($SELECT_RES as $SELECT_ROW) {
				...
				// echo "\r\n--DISPATCH ID::".$SELECT_ROW->id." FINAL_INV_AMOUNT ".$FINAL_INV_AMOUNT."--\r\n";
		...
		echo "\r\n--GRAND_TOTAL_NET_AMT -- ".$GRAND_TOTAL_NET_AMT."--\r\n";

		// $SELECT_SQL = "SELECT id FROM wm_service_master WHERE approval_status = 1 ORDER BY id DESC";
		// $SELECT_RES  = DB::connection()->select($SELECT_SQL);
		// if (!empty($SELECT_RES)) {
		// 	foreach ($SELECT_RES as $SELECT_ROW) {
		// 		echo "\r\n--SERVICE ID::".$SELECT_ROW->id."--\r\n";
		// 		$SQL_QUERY = "CALL SP_STORE_SERVICE_FINAL_AMT(".$SELECT_ROW->id.")";
		// 		DB::connection()->statement($SQL_QUERY);
		// 	}
		// }
