...
use DB;
class DailyProjectionPlan extends Model
{
	protected 	$table 		= 'wm_daily_projection_plan';
	...
	*/
	public static function GetProjectionPlans($request,$FromDetails = false)
	{
		try {
			...
			$sortBy 		= ($request->has('sortBy')  && !empty($request->sortBy)) ? $request->sortBy : "PLAN_CNT";
			...
			$SelectSql 		= self::select(	\DB::raw("$self.*"),
											...
											\DB::raw("CASE WHEN 1=1 THEN (SELECT COUNT(0) FROM wm_daily_projection_plan_details WHERE wm_daily_projection_plan_details.wm_daily_projection_plan_id = $self.id) END AS PLAN_CNT"),
			...
			if(($status < 0) || ($status > 0)){
				$SelectSql->where("$self.status",$status);
			}
			...
			if(!empty($request->input('projectiondate')) && !empty($request->input('projectiondate'))) {
				$projection_date = date("Y-m-d",strtotime($request->input('projectiondate')));
				$SelectSql->where("$self.projection_date",$projection_date);
			} else if(!empty($request->input('projection_date')) && !empty($request->input('projection_date'))) {
				$projection_date = date("Y-m-d",strtotime($request->input('projection_date')));
				$SelectSql->where("$self.projection_date",$projection_date);
			}
			if (empty($MRF_ID)) {
				if (!$FromDetails) {
			...
			if(isset($toArray['totalElements']) && $toArray['totalElements']>0) {
				foreach($toArray['result'] as $key => $value) {
					$PLAN_DETAILS 								= DailyProjectionPlanDetails::GetProjectionPlan($value['id']);
					$toArray['result'][$key]['PLAN_DETAILS'] 	= $PLAN_DETAILS;
					$toArray['result'][$key]['ShowDetails'] 	= (!empty($PLAN_DETAILS))?1:0;
					$toArray['result'][$key]['closing_stock'] 	= StockLadger::where("stock_date",$value['projection_date'])->where("product_type",PRODUCT_SALES)->where("product_id",$value['product_id'])->where("mrf_id",$value['mrf_id'])->value("opening_stock");
				}
			}
			return $toArray;
		}catch(\Exception $e) {
	...
	*/
	public static function addProjectionPlan($request)
	{
		...
		$projection_date= (isset($request->projection_date) && !empty($request->projection_date))?$request->projection_date:"";
		...
		if(!empty($plan_data))
		{
			foreach($plan_data as $key => $value)
			{
				$Add 				 = new self();
				$Add->mrf_id 		 = $mrf_id;
				$Add->projection_date= date("Y-m-d",strtotime($projection_date));
		...
		if($ProjectionPlan) {
			...
			if(!empty($plan_data))
			{
				foreach($plan_data as $key => $value)
				{
					...
					$ProjectionPlan->projection_date 	= $projection_date;
	...
	*/
	public function getProjectionPlanWidget($request)
	{
		...
		$PROJECTION_DATE 					= isset($request->projection_date)?$request->projection_date:date("Y-m-d");
		$COUNTER							= 0;
		...
		$arrResult[$COUNTER]["SALES_PLAN"] 	= $this->GetMRFProjectionPlan($MRF_ID,$PROJECTION_DATE);
		return $arrResult;
	}
	...
	Date 	: 22 Nov 2021
	*/
	private function GetMRFProjectionPlan($MRF_ID,$PROJECTION_DATE)
	{
		$arrResult 									= array();
		...
		$arrProducts 								= $this->GetSalesProjectionPlan($MRF_ID,$PROJECTION_DATE);
		$StartDate									= $PROJECTION_DATE;
		$EndDate									= $PROJECTION_DATE;
		foreach($arrProducts AS $RowID=>$PROJECT_ROW)
		{
			$PRODUCT_ID 										= $PROJECT_ROW['ID'];
			$arrResult['PRODUCTS'][$RowID] 						= $PROJECT_ROW;
			$ACTUAL_SALES_DETAILS								= $this->GetActualProductSalesByMRF($MRF_ID,$PRODUCT_ID,$PROJECTION_DATE);
		...
		$arrResult['ACTUAL_SP'] 				= (!empty($ACTUAL_TOTAL_AMT))?round(($ACTUAL_TOTAL_AMT / $ACTUAL_TOTAL_QTY),2):0;
	...
	Date 	: 22 Nov 2021
	*/
	private function GetSalesProjectionPlan($MRF_ID,$PROJECTION_DATE)
	{
		$self 			= (new static)->getTable();
		$AdminUser 		= new AdminUser();
		$Department 	= new WmDepartment();
		...
		$SelectSql 		= $this->select(\DB::raw("$self.*"),
										...
										\DB::raw("CPM.net_suit_code"),
										\DB::raw("CMS.department_name"),
										\DB::raw("CASE WHEN 1=1 THEN (SELECT SUM(qty) FROM wm_daily_projection_plan_details WHERE wm_daily_projection_plan_id = $self.id) END AS TOTAL_PROJECTION_QTY"),
										\DB::raw("CASE WHEN 1=1 THEN (SELECT SUM(rate * qty) FROM wm_daily_projection_plan_details WHERE wm_daily_projection_plan_id = $self.id) END AS TOTAL_PROJECTION_AMT"),
										\DB::raw("CONCAT(U1.firstname,' ',U1.lastname) as created_by_name"),
										\DB::raw("CONCAT(U2.firstname,' ',U2.lastname) as updated_by_name"));
		$SelectSql->join($Department->getTable()." AS CMS","$self.mrf_id","=","CMS.id");
		$SelectSql->join($CPM->getTable()." AS CPM","$self.product_id","=","CPM.id");
		...
		$SelectSql->where("$self.status",REQUEST_APPROVED);
		$SelectSql->where("$self.projection_date",$PROJECTION_DATE);
		...
		$GRAND_TOTAL_QTY 	= 0;
		$AVG_DAILY_PROD 	= 0;
		$YEAR 				= date("Y",strtotime($PROJECTION_DATE));
		$MONTH 				= date("m",strtotime($PROJECTION_DATE));
		...
		if (!empty($SelectRows)) {
			foreach($SelectRows as $RowID=>$SelectRow) {
				...
				$arrResult[$RowID]['PROJECTION_PLAN'] 		= DailyProjectionPlanDetails::GetProjectionPlan($SelectRow['id'],true);
	...
	Date 	: 22 Nov 2021
	*/
	private function GetActualProductSalesByMRF($MRF_ID,$PRODUCT_ID,$PROJECTION_DATE)
	{
		$StartDate	= date("Y-m-d",strtotime($PROJECTION_DATE));
