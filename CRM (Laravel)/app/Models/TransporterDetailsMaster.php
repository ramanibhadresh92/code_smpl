...
use Mail;
class TransporterDetailsMaster extends Model implements Auditable
{
	//
	...
	*/
	public static function ListTransporter($request,$isPainate=true){
		...
		if($request->has('params.po_for') && !empty($request->input('params.po_for')))
		{
			$data->where("$table.po_for", $request->input('params.po_for'));
		}
		if($request->has('params.po_no') && !empty($request->input('params.po_no')))
		{
			$data->where("TPDM.po_no", $request->input('params.po_no'));
		}
		if($request->has('params.paid_by_party') && !empty($request->input('params.paid_by_party')))
		{
			$data->where("$table.paid_by_party", $request->input('params.paid_by_party'));
		}
		if($request->has('params.transporter_name') && !empty($request->input('params.transporter_name')))
		{
			$data->where("TPDM.vendor_name",'like',"%".$request->input('params.transporter_name')."%");
		}
