...
use App\Models\WmDispatchEmailSendLog;
use Carbon\Carbon;
use App\Models\WmClientPurchaseOrders;
use App\Models\ViewCustomerMaster;
class WmDispatch extends Model implements Auditable
{
	...
	public function setTransporterInvoiceNoAttribute($value)
	{
		$this->attributes['transporter_invoice_no'] = (!empty($value) && $value != "null") ? $value : "";
	}
	public function setEprBilltNoAttribute($value)
	{
		$this->attributes['epr_billt_no'] = (!empty($value) && $value != "null") ? $value : "";
	}
	 public function setEprChallanNoAttribute($value)
	{
		$this->attributes['epr_challan_no'] = (!empty($value) && $value != "null") ? $value : "";
	}
	/*
	Use 	: List Dispatch
	Author 	: Axay Shah
	...
	*/
	public static function ListDispatch($request,$FromMobile = false)
	{
		...
		$is_delivery_challan    = ($request->has('params.is_delivery_challan')      && !empty($request->input('params.is_delivery_challan')))    ? $request->input('params.is_delivery_challan') 	: "";
		$from_widget    = ($request->has('params.from_widget')      && !empty($request->input('params.from_widget')))    ? $request->input('params.from_widget') 	: 0;
		$data = self::select("$Dispatch.*",
					...
					\DB::raw("(CASE WHEN $Dispatch.approval_status = 0 THEN 'Pending'
								...
								WHEN $Dispatch.approval_status = 4 THEN 'Delivery Challan Approved'
		...
		if(!empty($is_delivery_challan)){
			$is_delivery_challan = ($is_delivery_challan == "-1") ? 0 : $is_delivery_challan;
			$data->where("$Dispatch.is_delivery_challan",$is_delivery_challan);
		}
		...
		if($from_widget == 1){
			$data->where(function($query){
				// $base_location 	= GetUserAssignedBaseLocation(Auth()->user()->adminuserid);
				// prd(GetUserAssignedBaseLocation(Auth()->user()->adminuserid));
				$query->whereIn("MDI.base_location_id",GetUserAssignedBaseLocation(Auth()->user()->adminuserid));
		...
		if($FromMobile){
			...
			if($request->has('approval_status'))
			{
				...
				}elseif($request->input('approval_status') > 0){
					$data->where("$Dispatch.approval_status",$request->input('approval_status'));
				}
			}
		...
		if(!empty($result)){
			...
			if(isset($toArray['totalElements']) && $toArray['totalElements']>0){
				foreach($toArray['result'] as $key => $value){
					...
					$url 			= '';
					$invoiceEdit 	= 0;
					$challan_url 	= url('/getChallan')."/".passencrypt($toArray['result'][$key]['id']);
					...
					$toArray['result'][$key]['regenerate_invoice_flag'] = (!empty($InvoiceId) && Auth()->user()->adminuserid == 1 ? 0 : 0 );
					if(!empty($InvoiceId)){
						$url = url('/invoice')."/".passencrypt($InvoiceId)."/".passencrypt(1);
					}
					...
					$toArray['result'][$key]['invoice_url'] 			= ($value['approval_status'] == 4) ? $challan_url  : $url;
					...
					############# BILL FROM AND DESTINATION GST STATE CODE CONDITION AS DISCUSS WITH ACCOUNT TEAM ##################
					$isParentDispatch = WmDispatch::where("parent_dispatch_id",$value['id'])->count();
					if($TotalInvoiceAmount >= 0 && ($TotalInvoiceAmount > EWAY_BILL_MIN_AMOUNT || $value['dispatch_type'] == NON_RECYCLEBLE_TYPE ) || (!empty($BILL_DISPLAY_GST_CODE) && !empty($DEST_DISPLAY_GST_CODE) && $BILL_DISPLAY_GST_CODE != $DEST_DISPLAY_GST_CODE)){
						$toArray['result'][$key]['eway_bill_required']  = (!empty($value['eway_bill_no']) || $value['approval_status'] == 2 || $value['approval_status'] == 0) ? false : true;
						if(empty($value['eway_bill_no'])){
							$toArray['result'][$key]['invoice_download_flag'] = 0;
						}
					}
					if($isParentDispatch > 0){
						$toArray['result'][$key]['eway_bill_required']  = false;
					}
					...
					if($SALES_GST_AMT > 0 && (empty($value['e_invoice_no']) || $value['e_invoice_no'] == null) && !empty($CLIENT_GST)){
						$toArray['result'][$key]['invoice_download_flag'] 	= 0;
						$toArray['result'][$key]['eway_bill_upload_msg'] 	= trans("message.E_INVOICE_UPLOAD_MSG");;
					}
					...
					$MainApproval = AdminUserRights::checkUserAuthorizeForTrn(DISPATCH_RATE_APPROVAL_RIGHTS,Auth()->user()->adminuserid);
					if($MainApproval > 0){
						$toArray['result'][$key]['show_dispatch_approval'] 	= 1;
					}else{
						$VirtualApproval = AdminUserRights::checkUserAuthorizeForTrn(DISPATCH_VIRTUAL_RATE_APPROVAL_RIGHTS,Auth()->user()->adminuserid);
						if($VirtualApproval > 0 && $value['is_virtual'] == 1 && $value['dispatch_type'] == NON_RECYCLEBLE_TYPE){
							$toArray['result'][$key]['show_dispatch_approval'] 	= 1;
						}
					}
					#################### VIRTUAL MRF APPROVAL AND MAIN APPRLVAL RIGHTS LOGIC ################
	...
	public static function GetById($DispatchId,$ShowNameWithGST = false)
	{
		...
		$data =  self::with(["DispatchImages" =>function($q){
								$q->select("*",\DB::raw("'' as image_url"));
										...
										"WDAED.buyer_order_no",
										"WDAED.dispatch_doc_no",
										"WDAED.dispatched_through",
										"WDAED.bill_of_lading",
										"WDAED.terms_of_delivery",
										"WDAED.destination",
										"WDAED.delivery_note",
										"WDAED.delivery_note_date",
										"WDAED.other_reference",
										"WDAED.dated",
										"$Customer.mobile_no",
										"$Customer.zipcode",
										...
										"DEPT.signature as signature_stemp",
									...
									->leftjoin("wm_dispatch_addtional_edit_data as WDAED","$self.id","=","WDAED.dispatch_id")
		...
		if($data){
			...
			if(!empty($data['salesProduct'])) {
				foreach($data['salesProduct'] as $raw) {
					$raw->cogs_price 	= StockLadger::where("product_id",$raw->product_id)->where("product_type",PRODUCT_SALES)->where("mrf_id",$data->bill_from_mrf_id)->where("stock_date",date("Y-m-d"))->value("avg_price");
					$GST_NAME 			= "";
					if($ShowNameWithGST){
						$GST_NAME 	= ($FROM_SAME_STATE == "Y") ? "(CGST@".$raw->cgst."% SGST@".$raw->sgst."%)" : "(IGST@".$raw->igst."%)";
					}
					...
					$GET_RATE_DATA 				= WmProductMaster::GetSalesOrderClientRate($data->dispatch_date,$raw->product_id,$data->master_dept_id,$data->client_master_id,$OriginID);
			...
			$data['destination'] 			= $data->client_master_id;
	...
	*/
	public static function InsertDispatch($request){
		try{
			...
			$FROM_DELIVERY_CHALLAN 	= (isset($request['from_delivery_challan']) && !empty($request['from_delivery_challan'])) ? $request['from_delivery_challan'] : 0;
			$DISPATCH_IDS_DATA 		= (isset($request['dispatch_ids_data']) && !empty($request['dispatch_ids_data'])) ? $request['dispatch_ids_data'] : array();
			$ZERO_GROSS_AMOUNT 		= (isset($request['zero_gross_amount']) && !empty($request['zero_gross_amount'])) ? $request['zero_gross_amount'] : 0;
			$SEND_TO_EPR 			= (isset($request['send_to_epr']) && !empty($request['send_to_epr'])) ? $request['send_to_epr'] : 0;
			$epr_credit 			= (isset($request['epr_credit']) && !empty($request['epr_credit'])) ? $request['epr_credit'] : 0;

			$accepted_quantity = (!empty($request['accepted_quantity'])) ? _FormatNumberV2($request['accepted_quantity']) : 0 ;
			########## FINACIAL YEAR 2021 REGARDING CHANGES #########
			if(empty($TRANSFER_TRANS)) {
				return false;
			}
			############ DELIVERY CHALLAN CHANGES 9-01-2023 ############
			$IS_DELIVERY_CHALLAN = 0;
			if($ZERO_GROSS_AMOUNT == 0){
				$IS_DELIVERY_CHALLAN 	= WmClientMaster::where("id",$request['destination'])->value('only_delivery_challan');
			}

			$TRANSFER_TRANS 		= ($IS_DELIVERY_CHALLAN == 1 && $FROM_DELIVERY_CHALLAN == 0) ? DELIVERY_CHALLAN_FLAG : $TRANSFER_TRANS;
			############ DELIVERY CHALLAN CHANGES 9-01-2023 ############
			if(!empty($TRANSFER_TRANS)){
			...
			$Dispatch->accepted_quantity	= $accepted_quantity;
			...
			$Dispatch->eway_bill_no			= (isset($request['eway_bill_no']) && !empty($request['eway_bill_no']) && $request['eway_bill_no'] != "null") ? $request['eway_bill_no'] :'';
			...
			$Dispatch->send_to_epr 			   = $SEND_TO_EPR;
			$Dispatch->epr_credit 				= $epr_credit;
			...
			if($Dispatch->save()){
				...
				if($FROM_DELIVERY_CHALLAN == 1){
					$DISPATCH_IDS_DATA = (!empty($DISPATCH_IDS_DATA) && is_array($DISPATCH_IDS_DATA)) ? $DISPATCH_IDS_DATA : explode(",",$DISPATCH_IDS_DATA);
					self::whereIn("id",$DISPATCH_IDS_DATA)->update(["parent_dispatch_id" => $DISPATCH_ID]);
					/** ADDED BY KALPAK @since 2023-02-20 for Sales Target Widget */
					self::where("id",$DISPATCH_ID)->update(["is_from_delivery_challan" => $FROM_DELIVERY_CHALLAN]);
					/** ADDED BY KALPAK @since 2023-02-20 for Sales Target Widget */
				}
				...
				if(isset($request['sales_product']) && !empty($request['sales_product'])){
					...
					{
						WmDispatchProduct::where("id",$value['id'])->update(["accepted_quantity"=>$value['quantity']]);
					...
					if($SALES_ORDER_ID > 0 || $FROM_DELIVERY_CHALLAN == 1) {
						...
						$ApprovalRequest->request->add([
							...
							"from_delivery_challan" => $FROM_DELIVERY_CHALLAN
						]);
				...
				WmDispatchAddtionalData::EditDispatchAdditionalData($request,$DISPATCH_ID);
				$requestObj = json_encode($request,JSON_FORCE_OBJECT);
				LR_Modules_Log_CompanyUserActionLog($requestObj,$DISPATCH_ID);
			}
			...
			########## IMAGE WAY BRIDGE IN UPDATE DISPATCH ##############
			if(isset($request['epr_way_bridge_count']) && !empty($request['epr_way_bridge_count'])){
				WmDispatchMediaMaster::where("dispatch_id",$Dispatch->id)->where("media_type",PARA_WAYBRIDGE)->delete();
				for($i = 0; $i < $request['epr_way_bridge_count']; $i++){
					...
					$target_file 	= $path.basename($_FILES["epr_way_bridge_".$i]["name"]);
					$name 			= "epr_way_bridge_".$i.time().'.'. strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
					move_uploaded_file($name, $destinationPath);
					$media = WmDispatchMediaMaster::AddDispatchMedia($Dispatch->id,$name,$name,$path,PARA_WAYBRIDGE);
				}
			}
			if(isset($request['epr_billt_count']) && !empty($request['epr_billt_count'])){
				WmDispatchMediaMaster::where("dispatch_id",$Dispatch->id)->where("media_type",PARA_BILLT)->delete();
				for($i = 0; $i < $request['epr_billt_count']; $i++){
					$companyId 			= Auth()->user()->company_id;
					$path 				= PATH_COMPANY."/".PATH_DISPATCH;
					$destinationPath 	= public_path('/').$path;
					if(!is_dir($destinationPath)) {
					...
					$target_file 	= $path.basename($_FILES["epr_billt_".$i]["name"]);
					$name 			= "epr_billt_".$i.time().'.'. strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
					move_uploaded_file($name, $destinationPath);
					$media = WmDispatchMediaMaster::AddDispatchMedia($Dispatch->id,$name,$name,$path,PARA_BILLT);
				}
			}
			########## IMAGE WAY BRIDGE IN UPDATE DISPATCH ##############
			/*IF ENTRY FROM JOBWORK THEN UPDATE JOBWORK TABLE DATA OF THAT RELETED ID - 12 MARCH 2020*/
			...
			######### DELIVERY CHALLAN INVOICE GENERATION UPDATE ALL DOCUMENT FROM
			if($FROM_DELIVERY_CHALLAN == 1 && !empty($DISPATCH_IDS_DATA)){
				$STORE_DOC_FOR_INVOICE = WmDispatchMediaMaster::whereIn("dispatch_id",$DISPATCH_IDS_DATA)->get()->toArray();
				if(!empty($STORE_DOC_FOR_INVOICE)){
					foreach($STORE_DOC_FOR_INVOICE AS $SDFI_KEY => $SDFI_VAL){
						$media = WmDispatchMediaMaster::AddDispatchMedia($DISPATCH_ID,$SDFI_VAL['image_name'],$SDFI_VAL['image_name'],$SDFI_VAL['image_path'],$SDFI_VAL['media_type']);
						switch($SDFI_VAL['media_type']){
							case PARA_WAYBRIDGE :
								WmDispatch::where("id",$DISPATCH_ID)->update(array("epr_waybridge_slip_id" => $media));
								break;
							case PARA_BILLT :
								WmDispatch::where("id",$DISPATCH_ID)->update(array("epr_billt_media_id" => $media));
								break;
							case PARA_EWAY_BILL :
								WmDispatch::where("id",$DISPATCH_ID)->update(array("epr_ewaybill_media_id" => $media));
								break;
							case PARA_UNLOADING_SLIP :
								WmDispatch::where("id",$DISPATCH_ID)->update(array("unloading_slip_media_id" => $media));
								break;
							case PARA_TRANSPORTER_INV :
								WmDispatch::where("id",$DISPATCH_ID)->update(array("transporter_invoice_media_id" => $media));
								break;
							default :
							break;
						}
				...
				$MEDIA_RECORD_DATA = WmDispatch::select(
					\DB::raw("group_concat(DISTINCT epr_challan_no) as epr_challan_no"),
					\DB::raw("group_concat(DISTINCT epr_billt_no) as epr_billt_no"),
					\DB::raw("group_concat(DISTINCT epr_waybridge_no) as epr_waybridge_no"),
					\DB::raw("group_concat(DISTINCT unloading_slip_no) as unloading_slip_no"),
					\DB::raw("group_concat(DISTINCT transporter_invoice_no) as transporter_invoice_no")
				)->whereIn("id",$DISPATCH_IDS_DATA)->get()->toArray();
				if(!empty($MEDIA_RECORD_DATA)){
					foreach($MEDIA_RECORD_DATA AS $k => $v){
						$v_epr_challan_no 			= ltrim($v['epr_challan_no'],',');
						$v_epr_billt_no 			= ltrim($v['epr_billt_no'],',');
						$v_epr_waybridge_no 		= ltrim($v['epr_waybridge_no'],',');
						$v_unloading_slip_no 		= ltrim($v['unloading_slip_no'],',');
						$v_transporter_invoice_no 	= ltrim($v['transporter_invoice_no'],',');
						if(!empty($v_epr_challan_no)){
							self::where("id",$DISPATCH_ID)->update(array("epr_challan_no" =>$v_epr_challan_no));
						}
						if(!empty($v_epr_billt_no)){
							self::where("id",$DISPATCH_ID)->update(array("epr_billt_no" =>$v_epr_billt_no));
						}
						if(!empty($v_epr_waybridge_no)){
							self::where("id",$DISPATCH_ID)->update(array("epr_waybridge_no" =>$v_epr_waybridge_no));
						}
						if(!empty($v_unloading_slip_no)){
							self::where("id",$DISPATCH_ID)->update(array("unloading_slip_no" =>$v_unloading_slip_no));
						}
						if(!empty($v_transporter_invoice_no)){
							self::where("id",$DISPATCH_ID)->update(array("transporter_invoice_no" =>$v_transporter_invoice_no));
						}
					}
				}
			}
		...
		try{
			...
			/* GET TRANSFER TRANS AS PER TYPE OF TRANSACTION - 23 APRIL 2020 -AXAY SHAH */
				...
				if($Dispatch->dispatch_type != $DISPATCH_TYPE_TRN){
					...
					############ DELIVERY CHALLAN CHANGES 9-01-2023 ############
					$IS_DELIVERY_CHALLAN 	= WmClientMaster::where("id",$request['destination'])->value('only_delivery_challan');
					$TRANSFER_TRANS 		= ($IS_DELIVERY_CHALLAN == 1) ? DELIVERY_CHALLAN_FLAG : $TRANSFER_TRANS;
					############ DELIVERY CHALLAN CHANGES 9-01-2023 ############
					$GET_CODE 			= TransactionMasterCodesMrfWise::GetLastTrnCode($BILL_FROM_MRF_ID,$TRANSFER_TRANS);
					if($GET_CODE){
						$CODE 			= $GET_CODE->code_value + 1;
			...
			/* GET TRANSFER TRANS AS PER TYPE OF TRANSACTION - 23 APRIL 2020 -AXAY SHAH */
				...
				if($Dispatch->save()){
					...
					if($SALES_ORDER_ID > 0) {
						...
						$ApprovalRequest = new \Illuminate\Http\Request();
						$ApprovalRequest->request->add([
							'approval_status' 		=> ($IS_DELIVERY_CHALLAN == 1) ? 4 : 1,
							"challan_no" 			=> $challan_no,
				...
				WmDispatchAddtionalData::EditDispatchAdditionalData($request,$DispatchId);
				$requestObj = json_encode($request,JSON_FORCE_OBJECT);
	...
	*/
	public static function DispatchRateApproval($request,$fromMobile = false)
	{
		...
		$from_delivery_challan = (isset($request->from_delivery_challan) && !empty($request->from_delivery_challan)) ? $request->from_delivery_challan : 0;
		$TOTAL_PRICE 		= (isset($request->totalPrice) && !empty($request->totalPrice)) ? $request->totalPrice : 0;
		$WMProductPrice		= new WmProductClientPriceMaster();
		$CODE 				= 0;
		...
		if(!empty($DispatchId) && !empty($Status))
		{
			...
			if($DispatchData)
			{
				$directDispatch  	= isset($DispatchData->direct_dispatch) ? $DispatchData->direct_dispatch : 0;
				if($from_delivery_challan == 0 && $TOTAL_PRICE > 0){
					$DELIVERY_CHALLAN 	= WmClientMaster::where("id",$DispatchData->client_master_id)->value("only_delivery_challan");
				}
				...
				############ UPDATE RATE AS PER PER_TONNE_ACTUAL_CAPACITY & PER_TONNE_AT_VEHICLE_CAPACITY ##############
				$TOTAL_ACTUAL_QTY 	= ($DispatchData->quantity > 0) ? $DispatchData->quantity : 0;
				$TRANSPORTER_PO_DETAILS = TransporterDetailsMaster::where("id",$DISPATCH_PO_ID)->first();
				if($TRANSPORTER_PO_DETAILS && $from_delivery_challan == 0){
					$TRANSPORTER_PO_DETAILS->dispatch_id 	= $DispatchId;
					$TRANSPORTER_PO_DETAILS->ref_id 		= $DispatchId;
					$TRANSPORTER_PO_DETAILS->po_date 	 	= date("Y-m-d H:i:s");
					$TRANSPORTER_PO_DATA = TransporterPoDetailsMaster::find($TRANSPORTER_PO_DETAILS->po_detail_id);
					$RATE 				 = $TRANSPORTER_PO_DETAILS->rate;
					if($TRANSPORTER_PO_DETAILS->collection_partner == 1){
						$RATE = (isset($TRANSPORTER_PO_DETAILS->rate)) ? $TRANSPORTER_PO_DETAILS->rate * $TOTAL_ACTUAL_QTY : 0;
					}elseif($TRANSPORTER_PO_DATA && isset($TRANSPORTER_PO_DATA->vehicle_cost_type) && $TRANSPORTER_PO_DATA->vehicle_cost_type == PER_TONNE_ACTUAL_CAPACITY){
				...
				############ UPDATE RATE AS PER PER_TONNE_ACTUAL_CAPACITY & PER_TONNE_AT_VEHICLE_CAPACITY ##############
				############ TRANSPORTER PO MASTER DATA SEND TO BAMS WHEN INVOICE GOT APPROVE - 18-11-2022###########
				if(($Status == DISPATCH_APPROVED && $DISPATCH_PO_ID > 0) && $from_delivery_challan == 0){
					TransporterDispatchInvoiceProcessMaster::AddTransporterPOProcessRecord($DISPATCH_PO_ID,$DispatchId);
				}
				############ TRANSPORTER PO MASTER DATA SEND TO BAMS WHEN INVOICE GOT APPROVE ###########
				...
				if(!empty($Product))
				{
					...
					foreach($Product as $pro)
					{
						...
						if($Status == DISPATCH_REJECTED && $from_delivery_challan == 0 && $directDispatch == 0){
							$INWARDDATA['purchase_product_id'] 	= 0;
						...
						if($DispatchPlanID == 0 && $from_delivery_challan == 0){
				...
				if(!empty($DISPATCH_DATE_TIME) && strtotime($DISPATCH_DATE_TIME) >= $TCS_START_DATE){
					...
					$TOTAL_PRICE = WmDispatchProduct::where("dispatch_id",$DispatchId)->sum("gross_amount");
					if($from_delivery_challan == 0 && $TOTAL_PRICE > 0){
						$DELIVERY_CHALLAN 	= WmClientMaster::where("id",$DispatchData->client_master_id)->value("only_delivery_challan");
					}
					if($from_delivery_challan == 0 && $DELIVERY_CHALLAN == 1 && $request->approval_status == DISPATCH_APPROVED){
						$status = 4;
					}
				/*########## CALCULATE TCS AMOUNT###########*/
				...
				$update = 	self::where("id",$DispatchId)
								->update([
						"is_delivery_challan"	=>  $DELIVERY_CHALLAN,
				...
				if($update)
				{
					if(isset($challan_no) && !empty($challan_no))
					{
						...
						if($data)
						{
							...
							if($Status == DISPATCH_APPROVED &&  $DELIVERY_CHALLAN == 0) WmSalesMaster::GenerateInvoicev2($DispatchId);
				...
				$res['res_result'] 				= true;
				LR_Modules_Log_CompanyUserActionLog($request,$DispatchId);
				return $res;
			}
			$res['res_from_auto_einvoice'] 	= 0;
	...
	*/
	...
	public static function InsertDispatchWeb($request){
		...
		$SEND_TO_EPR 			= (isset($request['send_to_epr']) && !empty($request['send_to_epr'])) ? $request['send_to_epr'] : 0;
		$DISPATCH_IDS_DATA 		= (isset($request['dispatch_ids_data']) && !empty($request['dispatch_ids_data'])) ? $request['dispatch_ids_data'] : array();
		$product 				= (isset($request['sales_product']) && !empty($request['sales_product'])) ? json_decode($request['sales_product'],true) : array();
		$gross_amount 			= 0;
		$IS_DELIVERY_CHALLAN 	= 0;
		if(!empty($product)){
			$price 			= array_sum(array_column($product, 'price'));
			$quantity 		= array_sum(array_column($product, 'quantity'));
			$price 			= ($price > 0) ? $price : 0;
			$quantity 		= ($quantity > 0) ? $quantity : 0;
			$gross_amount 	= _FormatNumberV2($price * $quantity);
		}
		if($gross_amount > 0){
			$IS_DELIVERY_CHALLAN 	= WmClientMaster::where("id",$ClientId)->value('only_delivery_challan');
		}
		$TRANSFER_TRANS 		= TransactionMasterCodesMrfWise::GetTrnType($DISPATCH_TYPE_TRN);
		$TRANSFER_TRANS 		= ($IS_DELIVERY_CHALLAN == 1 && $FROM_DELIVERY_CHALLAN == 0) ? DELIVERY_CHALLAN_FLAG : $TRANSFER_TRANS;
		if(!empty($TRANSFER_TRANS)){
	...
	public static function DispatchReportV3($request)
	{
		...
		################ FOR REGISTER ITEM WISE REPORT ################
		$IS_DELIVERY_CHALLAN  	= (isset($request->is_delivery_challan)) ?  $request->is_delivery_challan : "";
		...
		if($request->has('invoice_generated')) {
			...
			} else if($invoice_generated == "1") {
				$DISPATCH_WHERE .= " AND wm_dispatch.invoice_generated = ".$invoice_generated;
			}
		}
		if($isFromDispatch == "0") {
			$DISPATCH_WHERE .= " AND wm_dispatch.id = 0";
		} else if($isFromDispatch == "1") {
			$TRANSFER_WHERE .= " AND wm_transfer_master.id = 0";
		}
		if($request->has('invoice_cancel')) {
			$invoice_cancel =  $request->input('invoice_cancel');
		...
		if($request->has('status')) {
			...
			if($status == "0") {
				...
				$TRANSFER_WHERE .= " AND NOT FIND_IN_SET(wm_transfer_master.approval_status,'0')";
			...
			$TRANSFER_WHERE .= " AND wm_transfer_master.approval_status IN (1,3,0) ";
		}
		if($IS_DELIVERY_CHALLAN > 0) {
			$DISPATCH_WHERE .= " AND wm_dispatch.is_delivery_challan = ".$IS_DELIVERY_CHALLAN;
			$TRANSFER_WHERE .= " AND wm_transfer_master.id = 0";
		}
		if(!empty($createdAt) && !empty($createdTo)) {
		...
		if($request->has('is_einvoice'))
		{
			...
			if($is_einvoice == "-1") {
				$DISPATCH_WHERE .= " AND wm_dispatch.acknowledgement_no IS NULL";
				$TRANSFER_WHERE .= " AND wm_transfer_master.ack_no IS NULL";
			}elseif($is_einvoice == 1){
				$DISPATCH_WHERE .= " AND wm_dispatch.acknowledgement_no IS NOT NULL";
				$TRANSFER_WHERE .= " AND wm_transfer_master.ack_no IS NOT NULL";
			}
		}
		if($request->has('send_to_epr'))
		{
			$send_to_epr = $request->input('send_to_epr');
			if($send_to_epr == 1) {
				$DISPATCH_WHERE .= " HAVING SentToEPR = 1 ";
				$TRANSFER_WHERE .= " HAVING SentToEPR = 1 ";
			} else if($send_to_epr == "0") {
				$DISPATCH_WHERE .= " HAVING SentToEPR = 0 ";
				$TRANSFER_WHERE .= " HAVING SentToEPR = 0 ";
			}
		}
		$DELIVERY_CHALLAN_REPORT_SQL = "";

		if ($IS_DELIVERY_CHALLAN > 0)
		{
			$DELIVERY_CHALLAN_REPORT_SQL = "SELECT
											wm_dispatch.parent_dispatch_id,
											wm_dispatch.is_delivery_challan,
											wm_dispatch.bill_from_mrf_id,
											wm_dispatch.e_invoice_no,
											wm_dispatch.acknowledgement_no,
											wm_dispatch.acknowledgement_date,
											...
											wm_product_master.sgst,
											wm_product_master.igst,
											wm_dispatch_product.product_id,
											wm_dispatch_product.cgst_rate,
											wm_dispatch_product.sgst_rate,
											wm_dispatch_product.igst_rate,
											wm_dispatch_product.gross_amount,
											wm_dispatch_product.net_amount,
											wm_dispatch_product.gst_amount,
											wm_dispatch_product.id as dispatch_product_id,
											wm_dispatch_product.quantity AS sales_quantity,
											wm_dispatch_product.price AS sales_rate,
											wm_department.department_name,
											BILL_FROM.department_name AS bill_from_mrf_name,
											wm_department.location_id,
											PARAM.para_value AS sales_type_name,
											...
											(
												...
												AND P_COGS.sales_product_id = wm_dispatch_product.product_id
												ORDER BY P_COGS.id LIMIT 1
											) END AS cogs,
											CASE WHEN 1=1 THEN
											(
												SELECT (IF(P_COGS.direct_dispatch = 1,P_COGS.price,P_COGS.avg_price) * wm_dispatch_product.quantity)
												FROM wm_dispatch_sales_product_avg_price as P_COGS
												WHERE P_COGS.dispatch_id = wm_dispatch.id
												AND P_COGS.sales_product_id = wm_dispatch_product.product_id
												ORDER BY P_COGS.id LIMIT 1
											...
											IF (wm_dispatch.aggregator_dispatch = 1,'PAID','MRF') AS sales,
											transporter_details_master.rate,
											transporter_details_master.demurrage,
											wm_dispatch.shipping_state,
											get_dispatch_cn_dn_revised_amount(wm_dispatch.id,0,wm_dispatch_product.product_id,wm_dispatch_product.id,0) as credit_revised_amt,
											get_dispatch_cn_dn_revised_amount(wm_dispatch.id,0,wm_dispatch_product.product_id,wm_dispatch_product.id,1) as credit_gst_revised_amt,
											get_dispatch_cn_dn_revised_amount(wm_dispatch.id,1,wm_dispatch_product.product_id,wm_dispatch_product.id,0) as debit_revised_amt,
											get_dispatch_cn_dn_revised_amount(wm_dispatch.id,1,wm_dispatch_product.product_id,wm_dispatch_product.id,1) as debit_gst_revised_amt,
											IF(lr_epr_mapping_master.dispatch_id IS NOT NULL AND lr_epr_mapping_master.epr_track_id IS NOT NULL,1,0) AS SentToEPR
											...
											LEFT JOIN `transporter_details_master` ON `wm_dispatch`.`transporter_po_id` = `transporter_details_master`.`id`
											LEFT JOIN `lr_epr_mapping_master` ON `wm_dispatch`.`id` = `lr_epr_mapping_master`.`dispatch_id`
											$DISPATCH_WHERE ";
		} else {
			$SALES_REPORT_SQL 		= "	SELECT
										...
										transporter_details_master.demurrage,
										wm_dispatch.shipping_state,
										get_dispatch_cn_dn_revised_amount(wm_sales_master.dispatch_id,0,wm_sales_master.product_id,wm_sales_master.dispatch_product_id,0) as credit_revised_amt,
										get_dispatch_cn_dn_revised_amount(wm_sales_master.dispatch_id,0,wm_sales_master.product_id,wm_sales_master.dispatch_product_id,1) as credit_gst_revised_amt,
										get_dispatch_cn_dn_revised_amount(wm_sales_master.dispatch_id,1,wm_sales_master.product_id,wm_sales_master.dispatch_product_id,0) as debit_revised_amt,
										get_dispatch_cn_dn_revised_amount(wm_sales_master.dispatch_id,1,wm_sales_master.product_id,wm_sales_master.dispatch_product_id,1) as debit_gst_revised_amt,
										IF(lr_epr_mapping_master.dispatch_id IS NOT NULL AND lr_epr_mapping_master.epr_track_id IS NOT NULL,1,0) AS SentToEPR
			...
			$TRANSFER_REPORT_SQL	= "	SELECT
										'0' as parent_dispatch_id,
										'0' as is_delivery_challan,
										...
										'0' as rate,
										'0' as demurrage,
										D_GST_STATE_CODES.state_name as shipping_state,
										'0' as credit_revised_amt,
										'0' as credit_gst_revised_amt,
										'0' as debit_revised_amt,
										'0' as debit_gst_revised_amt,
										'0' as SentToEPR
		...
		if ($request->is_from_dispatch == 1 || $request->is_from_dispatch == "1") {
			if($IS_DELIVERY_CHALLAN > 0){
				$SQL = $DELIVERY_CHALLAN_REPORT_SQL;
			}else{
		...
		if(!empty($result))
		{
			...
			foreach($result as $key => $value)
			{
				...
				if($value->is_from_dispatch == 1) {
					...
					########### CREDIT DEBIT NOTE CHANGES ###########
					if(Auth()->user()->adminuserid == 1) {
						$CREDIT_AMT 		= ($value->credit_revised_amt > 0) ? _FormatNumberV2($value->credit_revised_amt) : 0;
						$DEBIT_AMT 			= ($value->debit_revised_amt > 0) ?_FormatNumberV2($value->debit_revised_amt): 0;
						$CREDIT_GST_AMT 	= ($value->credit_gst_revised_amt > 0) ?_FormatNumberV2($value->credit_gst_revised_amt): 0;
						$DEBIT_GST_AMT 		= ($value->debit_gst_revised_amt > 0) ?_FormatNumberV2($value->debit_gst_revised_amt): 0;
					} else {
						$CREDIT_AMT 		= WmInvoicesCreditDebitNotesDetails::join($creditNoteTbl->getTable()."  as CRN_TBL","$detailsTbl.cd_notes_id","=","CRN_TBL.id")
												->where("CRN_TBL.dispatch_id",$value->dispatch_id)
												->where("CRN_TBL.notes_type",0)
				...
				$transportation_cost_rate 			= (isset($value->rate)) ? $value->rate : 0;
				$transportation_cost_demurrage 		= (isset($value->demurrage)) ? $value->demurrage : 0;
				$transportation_cost_total_price 	= ($transportation_cost_rate) + $transportation_cost_demurrage;
				$result[$key]->transportation_cost 	= $transportation_cost_total_price;
				...
				$DisDate 		= (!empty($value->Dispatch_Date)) ? date("Y-m-d",strtotime($value->Dispatch_Date)) : "";
				$new_cogs_price = StockLadger::where("mrf_id",$value->bill_from_mrf_id)->where("product_type",$value->product_type)->where("stock_date",$DisDate)->where("product_id",$value->product_id)->value("new_cogs_price");
				$result[$key]->new_cogs_price 		=  $new_cogs_price;
				$result[$key]->total_new_cogs_price =  _FormatNumberV2($new_cogs_price * $value->sales_quantity);;
				########## GET NEW COGS VALUE AND TOTAL AMOUNT - 14-07-2023 ##########
				...
				$result[$key]->gross_amount 		= _FormatNumberV2($GROSS_AMT);
				$result[$key]->product_description 	= $DESCRIPTION;
				$result[$key]->consignee_state 		= (isset($value) && !empty($value)) ? $value->shipping_state : "";;
				$result[$key]->client_state_name  	= GSTStateCodes::where("id",$value->gst_state_code)->value("state_name");
				$result[$key]->consignee_name  		= ShippingAddressMaster::where("id",$value->shipping_address_id)->value("consignee_name");
				$result[$key]->credit_gross_amt  	= "<font style='color:red;font-weight:bold'>".$CREDIT_AMT."</font>";
				$result[$key]->debit_gross_amt  	= "<font style='color:green;font-weight:bold'>".$DEBIT_AMT."</font>";
	...
	/** Dispatch Summary Report */
	public static function DispatchReportV4($request)
	{
		$base_location_Ids 	= (isset($request->base_location_id) && !empty($request->base_location_id)) ? $request->base_location_id : array() ;
		if(empty($base_location_Ids)){
			$cityId         = (isset($request->base_location_id) && !empty($request->base_location_id)) ? GetBaseLocationCity($request->base_location_id) : GetBaseLocationCity(Auth()->user()->base_location);
		...
		if(!empty($base_location_Ids)){
			...
			$DISPATCH_WHERE .= " AND FIND_IN_SET(wm_department.base_location_id,'".$BaseConcat."')";
			$TRANSFER_WHERE .= " AND FIND_IN_SET(O.base_location_id,'".$BaseConcat."')";
			...
			$INCLUDE_CITY 	= implode(",",$INCLUDE_CITY);
			$DISPATCH_WHERE .= " AND FIND_IN_SET(wm_department.location_id,'".$INCLUDE_CITY."')";
			$TRANSFER_WHERE .= " AND FIND_IN_SET(O.location_id,'".$INCLUDE_CITY."')";
		}
		if(!empty($EXCLUDE_CITY)) {
			$EXCLUDE_CITY 	= implode(",",$EXCLUDE_CITY);
			$DISPATCH_WHERE .= " AND NOT FIND_IN_SET(wm_department.location_id,'".$EXCLUDE_CITY."')";
			$TRANSFER_WHERE .= " AND NOT FIND_IN_SET(O.location_id,'".$EXCLUDE_CITY."')";
		}
		if(!empty($INCLUDE_BILL_MRF)) {
			$INCLUDE_BILL_MRF_ARR 	= $INCLUDE_BILL_MRF;
			$CN_DN_MRF_CON 			= array_merge($INCLUDE_BILL_MRF,$CN_DN_MRF_CON);
			...
			$DISPATCH_WHERE .= " AND FIND_IN_SET(wm_dispatch.bill_from_mrf_id,'".$INCLUDE_BILL_MRF."')";
			$TRANSFER_WHERE .= " AND FIND_IN_SET(wm_transfer_master.origin_mrf,'".$INCLUDE_BILL_MRF."')";
		...
		if(!empty($EXCLUDE_BILL_MRF)) {
			...
			$DISPATCH_WHERE .= " AND NOT FIND_IN_SET(wm_dispatch.bill_from_mrf_id,'".$EXCLUDE_BILL_MRF."')";
			$TRANSFER_WHERE .= " AND NOT FIND_IN_SET(wm_transfer_master.origin_mrf,'".$EXCLUDE_BILL_MRF."')";
		}
		...
		if($request->has('status')) {
			...
			if($status == "0") {
				...
				$DISPATCH_WHERE .= " AND wm_dispatch.approval_status =".$status;
				$TRANSFER_WHERE .= " AND NOT FIND_IN_SET(wm_transfer_master.approval_status,'3,".$status."')";
			} elseif($status == "2") {
				$DISPATCH_WHERE .= " AND wm_dispatch.approval_status =".$status;
				$TRANSFER_WHERE .= " AND wm_transfer_master.approval_status=".$status;

			}
		} else {
			$TRANSFER_WHERE .= " AND NOT FIND_IN_SET(wm_transfer_master.approval_status,'1,3')";
		}
		if($IS_DELIVERY_CHALLAN > 0) {
			$DISPATCH_WHERE .= " AND wm_dispatch.is_delivery_challan = ".$IS_DELIVERY_CHALLAN;
			$TRANSFER_WHERE .= " AND wm_transfer_master.id = 0";
		...
		if ($request->is_from_dispatch == 1 || $request->is_from_dispatch == "1") {
			if($IS_DELIVERY_CHALLAN > 0){
				$SQL = 'CALL SP_DISPATCH_DELIVERY_REPORT("'.$DISPATCH_WHERE.'")';
			}else{
				$SQL = 'CALL SP_DISPATCH_SALES_REPORT("'.$DISPATCH_WHERE.'")';
			}
		} else if ($request->is_from_dispatch == "0") {
			$SQL = 'CALL SP_DISPATCH_TRNSFER_SALES_REPORT("'.$TRANSFER_WHERE.'")';
		} else {
		...
		if(!empty($result))
		{
			...
			foreach($result as $key => $value)
			{
				...
				// if (Auth()->user()->adminuserid == 1) {
				// 	$file 				= storage_path("/dispatch-data.txt");
				// 	$fp 				= fopen($file,"a+");
				// 	fwrite($fp,",".$value->dispatch_id);
				// 	fclose($fp);
				// }
				...
				if($value->is_from_dispatch == 1) {
					...
					if($value->product_type == 1)
					{
						$PRODUCT_DATA 				= CompanyProductMaster::select("company_product_master.*",\DB::raw("concat(company_product_master.name,' ',company_product_quality_parameter.parameter_name) as product_name"))->leftjoin("company_product_quality_parameter","company_product_master.id","=","company_product_quality_parameter.product_id")->where("company_product_master.id",$value->product_id)->first();
						$result[$key]->productName 	= ($PRODUCT_DATA) ? $PRODUCT_DATA->product_name : "";
						$result[$key]->title 		= ($PRODUCT_DATA) ? $PRODUCT_DATA->product_name : "";
						$result[$key]->hsn_code 	= ($PRODUCT_DATA) ? $PRODUCT_DATA->hsn_code : "";
						$result[$key]->net_suit_code= ($PRODUCT_DATA) ? $PRODUCT_DATA->net_suit_code : "";
	...
	public static function DispatchDeliveryChallanReport($DISPATCH_WHERE=""){
		$SALES_REPORT_SQL 		= "	SELECT
									wm_dispatch.parent_dispatch_id,
									wm_dispatch.is_delivery_challan,
									wm_dispatch.bill_from_mrf_id,
									...
									IF (wm_dispatch.aggregator_dispatch = 1,'PAID','MRF') AS sales
									FROM `wm_dispatch_product`
									LEFT JOIN `wm_sales_master` ON `wm_dispatch_product`.`id` = `wm_sales_master`.`wm_sales_master`
									LEFT JOIN `wm_product_master` ON `wm_sales_master`.`product_id` = `wm_product_master`.`id`
									LEFT JOIN `wm_dispatch` ON `wm_dispatch`.`id` = `wm_sales_master`.`dispatch_id`
									LEFT JOIN `wm_client_master` ON `wm_client_master`.`id` = `wm_dispatch`.`client_master_id`
									LEFT JOIN `parameter` AS `PARAM` ON `wm_dispatch`.`type_of_transaction` = `PARAM`.`para_id`
	...
	*/
	public static function UpdateDocumentForEPR($request,$dispatchID=0)
	{
		...
		$declaration_flag 	=  (isset($request->declaration_flag) && !empty($request->declaration_flag)) ?  $request->declaration_flag : 0;
		$declaration_date 	=  (isset($request->declaration_date) && !empty($request->declaration_date)) ?  date("Y-m-d",strtotime($request->declaration_date)) : date("Y-m-d");
		...
		$declaration_data 		= new WmTransportationDeclarationMaster();
		$vehicleData 			= VehicleMaster::where("vehicle_id",$GET_IMAGES->vehicle_id)->first();
		$clientData 			= WmClientMaster::where("id",$GET_IMAGES->client_master_id)->first();
		$sourceData 			= LocationMaster::where("location_id",$GET_IMAGES->origin_city)->first();
		$destinationData 		= LocationMaster::where("location_id",$GET_IMAGES->destination_city)->first();
		$declaration_count 		= WmTransportationDeclarationMaster::where('dispatch_id',$DISPATCH_ID)->count();
		if($declaration_flag == 1){
			if($declaration_count == 0){
				$declaration_data->vehicle_number 			= $vehicleData->vehicle_number;
				$declaration_data->declaration_date 		= $declaration_date;
				$declaration_data->client 					= $clientData->client_name;
				$declaration_data->source 					= $sourceData->city;
				$declaration_data->destination 				= $destinationData->city;
				$declaration_data->type_of_transportation 	= " ";
				$declaration_data->transporter 				= $GET_IMAGES->transporter_name;
				$declaration_data->waste_type 				= " ";
				$declaration_data->dispatch_id 				= $DISPATCH_ID;
				$declaration_data->status 					= $declaration_flag;
				if($declaration_data->save()){
					$COMPANY_ID = Auth()->user()->company_id;
					$FILENAME 	= "dispatch_declaration_".$declaration_data->id;
					$file_name 	= passencrypt($FILENAME);
					$file_name 	= $FILENAME;
					$PATH_COMPANY = PATH_IMAGE."/".PATH_COMPANY.'/'.$COMPANY_ID."/";
					$PATH 		= PATH_DISPATCH ;
					if(!is_dir(public_path($PATH_COMPANY))) {
						mkdir(public_path($PATH_COMPANY).$PATH,0777,true);
					}
					$file_url	= public_path($PATH_COMPANY).$PATH.'/';
					$data 		= WmTransportationDeclarationMaster::generateBillTDeclarationPDF($declaration_data->id);
					$PDF 		= PDF::loadView('pdf.declaration-billt',$data);
					$PDF->setPaper("letter","A4");
					$PDF->save($file_url.$FILENAME.".pdf");
					$DeclarationMediaId = WmDispatchMediaMaster::AddDispatchMedia($DISPATCH_ID,$FILENAME.'.pdf',$FILENAME.'.pdf',$PATH_COMPANY.$PATH,PARA_BILLT);
					self::where("dispatch_id",$DISPATCH_ID)->update(["epr_billt_no"=>"NA","epr_billt_media_id"=>$DeclarationMediaId]);
				}
			}else if($declaration_count == 1){
				WmTransportationDeclarationMaster::where("dispatch_id",$DISPATCH_ID)->update(["status"=>1]);
			}
		}else if($declaration_flag == 0){
			if($declaration_count == 1){
				WmTransportationDeclarationMaster::where("dispatch_id",$DISPATCH_ID)->update(["status"=>0]);
			}
		}
		return $DISPATCH_ID;
	}
	...
	*/
	...
	Date 	: 15 April,2020
	*/
	public static function SendDataToEPR28022023($request)
	{
		$WmDispatchProductTbl 	= new WmDispatchProduct();
		...
		$RAW 					= array();
		$RESPONSE 				= array();
		$data 					= self::select(	"$self.challan_no",
												"$self.id",
												"$self.company_id",
												"$self.dispatch_date",
												"$self.origin",
		...
		$RES = $data->whereIn("$self.approval_status",array(1))->orderBy("id","ASC")->first();
		if(!$RES) {
			return $RAW;
		}
		$RAW = $RES->toArray();
		if(!empty($RAW)) {
			...
			$FLEXI_CNT = WmDispatchProduct::select("id")->whereIn("product_id",FLEXI_PRODUCT_ARRAY)->where("dispatch_id",$RAW['id'])->count();
			$RIGID_CNT = WmDispatchProduct::select("id")->whereIn("product_id",RIGID_PRODUCT_ARRAY)->where("dispatch_id",$RAW['id'])->count();
			if ($FLEXI_CNT > 0 && $RIGID_CNT > 0) {
				$result = array();
				return $result;
			}
			if(!empty($RAW['epr_challan_media_id'])) {
				$CHALLAN_MEDIAS = WmDispatchMediaMaster::select("dispatch_id","image_name","image_path","id")->where("id",$RAW['epr_challan_media_id'])->get();
			...
			if(!empty($RAW['epr_billt_media_id'])) {
				$BILLT_MEDIAS 	= WmDispatchMediaMaster::select("dispatch_id","image_name","image_path","id")
									->where("dispatch_id",$RAW['id'])
									->where("media_type",PARA_BILLT)
									->get();
			...
			if(!empty($RAW['epr_waybridge_slip_id'])) {
				$WAYBRIDGE_MEDIAS 	= WmDispatchMediaMaster::select("dispatch_id","image_name","image_path","id")
										->where("dispatch_id",$RAW['id'])
										->where("media_type",PARA_WAYBRIDGE)
										->get();
			...
			if(!empty($RAW['epr_ewaybill_media_id'])) {
				$EWAYBILL_MEDIAS 	= WmDispatchMediaMaster::select("dispatch_id","image_name","image_path","id")
										->where("dispatch_id",$RAW['id'])
										->where("media_type",PARA_EWAY_BILL)
										->get();
			...
			if(!empty($RAW['unloading_slip_media_id'])) {
				...
				if(!empty($UNLOADING_MEDIAS)) {
					foreach ($UNLOADING_MEDIAS as $UNLOADING_MEDIA) {
						$UNLOADINGMEDIA['type'] 		= CHALLAN_TYPE;
						$UNLOADINGMEDIA['image_name'] 	= $UNLOADING_MEDIA->image_name;
						$UNLOADINGMEDIA['image_url'] 	= url('/')."/".$UNLOADING_MEDIA->image_path."/".$UNLOADING_MEDIA->image_name;
						array_push($DISPATCH_IMAGES_ARR,$UNLOADINGMEDIA);
					}
				}
			}
			if(!empty($RAW['transporter_invoice_media_id'])) {
				$TRANSPORTER_INV_MEDIAS = WmDispatchMediaMaster::select("dispatch_id","image_name","image_path","id")
											->where("dispatch_id",$RAW['id'])
											->where("media_type",PARA_TRANSPORTER_INV)
											->get();
				if(!empty($TRANSPORTER_INV_MEDIAS)) {
					foreach ($TRANSPORTER_INV_MEDIAS as $TRANSPORTER_MEDIA) {
						$TRANSPORTERMEDIA['type'] 		= TRANSPORTER_INV_TYPE;
						$TRANSPORTERMEDIA['image_name'] = $TRANSPORTER_MEDIA->image_name;
						$TRANSPORTERMEDIA['image_url'] 	= url('/')."/".$TRANSPORTER_MEDIA->image_path."/".$TRANSPORTER_MEDIA->image_name;
						array_push($DISPATCH_IMAGES_ARR,$TRANSPORTERMEDIA);
					}
				}
			}
			$InvoiceId 	= WmInvoices::where("dispatch_id",$request->dispatch_id)->value('id');
			if(file_exists(public_path("/").PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/"."invoice_".$InvoiceId.".pdf")) {
				if(isset($RAW['dispatch_date']) && !empty($RAW['dispatch_date']) && date("Y-m-d",strtotime($RAW['dispatch_date'])) < EPR_DIGI_SIGNATURE_START_DATE) {
			...
			if ($FLEXI_CNT > 0) {
				$MATERIAL_TYPE = EPR_FLEXI_ID;
			} else if ($RIGID_CNT > 0) {
				$MATERIAL_TYPE = EPR_RIGID_ID;
			}
	...
	public static function SendDataToEPR($request)
	{
		...
		$Location 				= new LocationMaster();
		$EPR 					= new LrEprMappingMaster();
		$TransporterDetails 	= new TransporterDetailsMaster();
		$WmDispatchProduct 		= $WmDispatchProductTbl->getTable();
		$CustomerMaster 		= $CustomerMasterTbl->getTable();
		$Sales 					= $SalesTbl->getTable();
		$Product				= $ProductTbl->getTable();
		...
		$data 					= self::select(	"$self.challan_no",
												...
												"LOC1.city as destination_city_name",
												"EPR.epr_track_id",
												"TransporterDetails.epr_po_no"
											)->with(["DispatchSalesData" => function($q) use($Product,$Sales){
												$q->join($Product." as PRO",$Sales.".product_id","=","PRO.id");
												$q->select("$Sales.*","PRO.title AS product_name","PRO.hsn_code");
											}])->with(["DispatchProductData" => function($q) use($Product,$WmDispatchProduct){
												$q->join($Product." as PRO",$WmDispatchProduct.".product_id","=","PRO.id");
												$q->select("$WmDispatchProduct.*","PRO.title AS product_name","PRO.hsn_code");
											}])
										->leftjoin($Parameter->getTable()." AS PARAM","$self.dispatch_type","=","PARAM.para_id")
										->leftjoin($Vehicle->getTable()." AS V","$self.vehicle_id","=","V.vehicle_id")
										->leftjoin($Client->getTable()." AS CLIENT","$self.destination","=","CLIENT.id")
										...
										->leftjoin($Location->getTable()." AS LOC","MRF.location_id","=","LOC.location_id")
										->leftjoin($Location->getTable()." AS LOC1","$self.destination_city","=","LOC1.location_id")
										->leftjoin($TransporterDetails->getTable()." AS TransporterDetails","$self.id","=","TransporterDetails.dispatch_id")
										->leftjoin($EPR->getTable()." AS EPR","$self.id","=","EPR.dispatch_id");
		if(!empty($request->dispatch_id)) {
			$data->where("$self.id",$request->dispatch_id);
		}
		$RES = $data->whereIn("$self.approval_status",array(1,4))->orderBy("id","ASC")->first();
		if(!$RES) {
			return $RAW;
		}
		$RAW = $RES->toArray();
		if(!empty($RAW))
		{
			$hide_price 			= ($RAW['epr_credit'] == 1) ? 1 : 0;
			$DISPATCH_ID 			= $RAW['id'];
			$DISPATCH_IMAGES_ARR 	= array();
			/** NOT REQUIRED TO SEND TO EPR */
			$FLEXI_CNT = WmDispatchProduct::select("id")->whereIn("product_id",FLEXI_PRODUCT_ARRAY)->where("dispatch_id",$RAW['id'])->count();
			...
			if(!empty($RAW['epr_ewaybill_media_id'])) {
				...
				if($RAW['epr_credit'] == 1){
					$EWAYBILL_MEDIAS = WmDispatchMediaMaster::select("dispatch_id","image_name","image_path","id")
										->where("dispatch_id",$RAW['id'])
										->where("media_type",PARA_STRICK_OUT_EWAY_BILL)->get();
				}
			...
			if($RAW['approval_status'] == 4)
			{
				$FROM_EPR 			= 1;
				$PATH_TO_COPY 		= public_path("/").PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/".PATH_CHALLAN;
				######### SEND ZERO AMOUNT WHEN EPR CREDIT IS 1 ################

				if($RAW['epr_credit'] == 1){
					$PATH_TO_COPY 		= public_path("/").PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/".PATH_CHALLAN."/".PATH_EPR;
				}
				...
				$PDF_NAME			= $PATH_TO_COPY."/delivery_challan_".$RAW['id'].".pdf";
				$IMG_NAME			= $PATH_TO_COPY."/delivery_challan_".$RAW['id'].".jpg";
				$data       		= WmDispatch::GetById($RAW['id']);
				$pdf        		= PDF::loadView('pdf.challan',compact('data','hide_price'));
				$pdf->setPaper("A4", "potrait");
				$pdf->stream("one");
				$pdf->save($PDF_NAME);
				$BILL_OF_SUPPLY['image_url'] 	=  url('/')."/".PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/".PATH_CHALLAN."/delivery_challan_".$RAW['id'].".pdf";
				######### SEND ZERO AMOUNT WHEN EPR CREDIT IS 1 ################
				if($RAW['epr_credit'] == 1) {
					$BILL_OF_SUPPLY['image_url'] 	=  url('/')."/".PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/".PATH_CHALLAN."/".PATH_EPR."/delivery_challan_".$RAW['id'].".pdf";
				}
				######### SEND ZERO AMOUNT WHEN EPR CREDIT IS 1 ################
				$BILL_OF_SUPPLY['type'] 		=  BILL_OF_SUPPLY;
				$BILL_OF_SUPPLY['image_name'] 	=  "delivery_challan_".$RAW['id'].".pdf";
				array_push($DISPATCH_IMAGES_ARR,$BILL_OF_SUPPLY);
			} else {
				$InvoiceId 	= WmInvoices::where("dispatch_id",$DISPATCH_ID)->value('id');
				if(file_exists(public_path("/").PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/"."invoice_".$InvoiceId.".pdf") && $RAW['epr_credit'] == 0) {
					if(isset($RAW['dispatch_date']) && !empty($RAW['dispatch_date']) && date("Y-m-d",strtotime($RAW['dispatch_date'])) < EPR_DIGI_SIGNATURE_START_DATE) {
						$PATH_TO_COPY 		= public_path("/").PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH;
						$PDF_NAME			= $PATH_TO_COPY."/invoice_".$InvoiceId.".pdf";
						$IMG_NAME			= $PATH_TO_COPY."/invoice_".$InvoiceId.".jpg";
					...
					######### SEND ZERO AMOUNT WHEN EPR CREDIT IS 1 ################
					if($RAW['epr_credit'] == 1) {
						$PATH_TO_COPY 		= public_path("/").PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/".PATH_EPR;
						if(!is_dir($PATH_TO_COPY)) {
							mkdir($PATH_TO_COPY,0777,true);
						}
					}
					...
					$IMG_NAME			= $PATH_TO_COPY."/invoice_".$InvoiceId.".jpg";
					$data       		= WmInvoices::GetById($InvoiceId);
					$pdf        		= PDF::loadView('pdf.one',compact('data','FROM_EPR','hide_price'));
					$pdf->setPaper("A4", "potrait");
					$pdf->stream("one");
					$pdf->save($PDF_NAME);
				}
				######### SEND ZERO AMOUNT WHEN EPR CREDIT IS 1 ################
				if($RAW['epr_credit'] == 1) {
					$BILL_OF_SUPPLY['image_url'] 	=  $BILL_OF_SUPPLY['image_url'] 	=  url('/')."/".PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/".PATH_EPR."/invoice_".$InvoiceId.".pdf";;
				} else {
					$BILL_OF_SUPPLY['image_url'] 	=  url('/')."/".PATH_IMAGE."/".PATH_COMPANY."/".$RAW['company_id']."/".PATH_DISPATCH."/invoice_".$InvoiceId.".pdf";
				}
				######### SEND ZERO AMOUNT WHEN EPR CREDIT IS 1 ################
			...
			/** GENERATE BILL OF SUPPLY */
			/** SEND DISPATCH BRANDING IMAGE TO EPR */
			$BRANDING_IMG_IDS 	= WmDispatchBrandingImgMapping::GetImageMediaIds($DISPATCH_ID);
			$BRANDING_INV_DATA 	= array();
			if(!empty($BRANDING_IMG_IDS)) {
				$BRANDING_INV_DATA 	= WmDispatchMediaMaster::select("dispatch_id","image_name","image_path","id")->whereIn("id",$BRANDING_IMG_IDS)->get();
				if(!empty($BRANDING_INV_DATA)) {
			...
			$RAW['dispatch_images'] = $DISPATCH_IMAGES_ARR;
			if($RAW['approval_status'] == 4) {
				if(!empty($RAW['dispatch_product_data'])) {
					foreach($RAW['dispatch_product_data'] as $raw) {
						$TOTAL_QTY 		+= (!empty($raw['quantity'])) ? _FormatNumberV2($raw['quantity']) : 0;
						$TOTAL_GROSS 	+= (!empty($raw['gross_amount'])) ? _FormatNumberV2($raw['gross_amount']) : 0;
						$TOTAL_NET 		+= (!empty($raw['net_amount'])) ? _FormatNumberV2($raw['net_amount']) : 0;
						$TOTAL_GST 		+= (!empty($raw['gst_amount'])) ? _FormatNumberV2($raw['gst_amount']) : 0;
			...
			$RESPONSE['dispatch_date'] 					= date("Y-m-d H:i:s",strtotime($RAW['dispatch_date']));
			$RESPONSE['epr_recycler_type'] 				= $RAW['epr_recycler_type'];
			$RESPONSE['epr_po_no'] 						= isset($RAW['epr_po_no']) && !empty($RAW['epr_po_no'])?$RAW['epr_po_no']:""; //Added By Kalpak @Since 2023-10-31
			$MATERIAL_TYPE 								= $RAW['material_type'];
			if ($FLEXI_CNT > 0) {
				$MATERIAL_TYPE = EPR_FLEXI_ID;
			} else if ($RIGID_CNT > 0) {
	...
	*/
	public static function CallEPRUrl()
	{
		...
		$FLEXI_RIGID_DATE 		= "2022-10-01 00:00:00";
		$DispatchIDs 			= array();
		$EPR_PRODUCT_ID 		= array_merge(FLEXI_PRODUCT_ARRAY,RIGID_PRODUCT_ARRAY);
		$GET_LAST_DISPATCH_ID 	= self::select(	"$EPR.dispatch_id",
												"$self.id",
												"$self.challan_no",
												"$self.epr_challan_media_id",
												"$self.dispatch_type",
									...
									->leftjoin($Client->getTable()." AS CLIENT","$self.destination","=","CLIENT.id")
									->where("$self.is_from_delivery_challan",0)
									->where("$self.client_master_id","!=",1762)
									->whereNotIn("$self.id",array(51438,51447,51484,51495,51525,51732,51739,51812,51629,51733,51741,51813))
									->where("CLIENT.gstin_no","!=","")
									->where(function($q) use ($self,$startDate){
										return $q->where("$self.strick_out_eway_bill_id",">",0)->
										orWhere("$self.dispatch_date",">=",$startDate)
										->orWhere("$self.epr_credit",1);
									})
		...
		if(!empty($DISPATCH_ROWS))
		{
			foreach($DISPATCH_ROWS AS $RAW)
			{
				...
				$count  				= self::where("parent_dispatch_id",$RAW->id)->count();
				if($count == 0){
	...
	public static function UpdateChallanToEPR()
	{
		...
		if(!empty($GET_LAST_DISPATCH_ID)) {
			foreach($GET_LAST_DISPATCH_ID AS $RAW) {
				$unloadinSlipCnt 		= WmDispatchMediaMaster::where("dispatch_id",$RAW->dispatch_id)->WHERE("media_type",PARA_UNLOADING_SLIP)->count();
				if($unloadinSlipCnt > 0){
					$INSERTED_ID 			= 0;
					$request 				= new Request();
					...
					if(!empty($data)) {
						...
						$url 		= EPR_URL;
						$client 	= new \GuzzleHttp\Client(['headers' => ['Content-Type' => 'application/json']]);
						$response 	= $client->request('POST', $url,['form_params' => [$request_data]]);
						$response 	= $response->getBody()->getContents();
						$res 		= json_decode($response,true);
						if(!empty($res)) {
							$IDS 		= (isset($res['Data']) && $res['Data'] > 0) ? $res['Data'] : 0;
							$Process 	= (isset($res['Data']) && $res['Data'] > 0) ? 2 : 1;
							LrEprMappingMaster::where("id",$INSERTED_ID)->update(["response_data"=>$response,"epr_track_id"=>$IDS,"process"=>$Process]);
						}
					}
				}
			}
	...
	*/
	public static function getEprDocument($DispatchId=0)
	{
		...
		if($Dispatch){
			...
			$declaration_count 			= WmTransportationDeclarationMaster::where('dispatch_id',$DispatchId)->where('status','1')->count();
			$result['declaration_flag'] = ($declaration_count == 1) ? 1 : 0;
			$result['can_upload_after_declaration'] = ($declaration_count == 1) ? 0 : 1;
			$result['declaration_date'] = WmTransportationDeclarationMaster::where('dispatch_id',$DispatchId)->value('declaration_date');
	...
	*/
	public static function GenerateEwayBillFromDispatch($id){
		...
		if($data){
			...
			if(isset($data['is_delivery_challan']) && $data['is_delivery_challan'] == 1){
				$REQUEST_DATA['docType']    = "CHL";
				$REQUEST_DATA['subSupplyType']  = "8";
				$REQUEST_DATA['subSupplyDesc']  = "Delivery Challan";
			}
			...
			$ArrProduct = WmDispatchProduct::select("wm_dispatch_product.*","wm_dispatch_product.price as rate","wm_product_master.id as product_id","wm_product_master.title AS product_name","wm_product_master.hsn_code")
						->leftjoin("wm_product_master","wm_dispatch_product.product_id","=","wm_product_master.id")
						->where("wm_dispatch_product.dispatch_id",$id)
						->get()
						->toArray();
			$itemList 			= array();
			$Amount 			= 0;
			...
			$TRANSPOTER_PO_ID 	= (isset($data['transporter_po_id']) && !empty($data['transporter_po_id'])) ? $data['transporter_po_id'] :0;
			$TRANSPORTER_GST_NO = "";
			$TRANSPORTER_DATA 	= TransporterDetailsMaster::find($TRANSPOTER_PO_ID);
			$PAID_BY_PARTY 		= ($TRANSPORTER_DATA) ? $TRANSPORTER_DATA->paid_by_party : "";
			$TRANSPORTER_GST_NO = (isset($TRANSPORTER_DATA->po_detail_id) && ($TRANSPORTER_DATA->po_detail_id > 0)) ? TransporterPoDetailsMaster::where("id",$TRANSPORTER_DATA->po_detail_id)->value("gst_no") : "";

			if(!empty($TRANSPORTER_NAME)){
				if(strtolower(str_replace(" ","",$TRANSPORTER_NAME)) == "thirdparty" || $PAID_BY_PARTY == 1 ){
					$TRANSPORTER_NAME = "";
				}
			}
			################ TRANSPOTER DETAILS ######################
			...
			$REQUEST_DATA['transporterId'] 		= (!empty($TRANSPORTER_GST_NO) && $TRANSPORTER_GST_NO != "null") ? $TRANSPORTER_GST_NO : "";
	...
	*/
	public static function GenerateEInvoice($id){
		...
		if($data){
			...
			############# CHARGES DETAILS PUSH IN E INVOICE IF APPLICABLE FOR Freight - 14 APRIL 2023 ##############
			$FREIGHT_CHARGES_DATA = InvoiceAdditionalCharges::GetFreightDataForEInvoice($id);
			if(!empty($FREIGHT_CHARGES_DATA)){
				$count = sizeof($itemList);
				$TOTAL_TAXABLE_AMT 	+= $FREIGHT_CHARGES_DATA['taxableAmount'];
				$TOTAL_AMOUNT       += $FREIGHT_CHARGES_DATA['totalItemAmount'];
				$TOTAL_TAX_AMOUNT 	+= $FREIGHT_CHARGES_DATA['gstAmount'];
				if($IsFromSameState){
					$TOTAL_CGST 		+= $FREIGHT_CHARGES_DATA['cgstAmt'];
					$TOTAL_SGST 		+= $FREIGHT_CHARGES_DATA['sgstAmt'];
				}else{
					$TOTAL_IGST 		+= $FREIGHT_CHARGES_DATA['igstAmt'];
				}
				$itemList[$count] 	= $FREIGHT_CHARGES_DATA;
				$count++;
			}
			$RENT_AMT 		=  0;
			$RENT_GST_AMT 	=  0;
			...
			$array["DocDtls"]["Typ"]            = ($TOTAL_TAXABLE_AMT > 0) ?  'INV' : 'BIL' ;
	...
	*/
	public static function UpdateAggregetorDispatchFlag($request)
	{
		...
		$data 			= false;
		if($DispatchID > 0) {
			$count 	= WmDispatch::select("id")->where("id",$DispatchID)->where("virtual_target",1)->count();
			if ($count == 0) {
				$data = WmDispatch::where("id",$DispatchID)->update(["aggregator_dispatch" => $aggregatorFlag]);
			} else {
				$count 	= WmDispatch::select("id")->where("id",$DispatchID)->count();
				if ($count > 0) {
					$data = 3;
				}
			}
	...
	*/
	...
	public static function AggregatorPLReport($request)
	{
		...
		$data 					= WmDispatch::select(	"$APP.app_date_time",
														...
														\DB::raw("WDP.epr_rate"))
									->join($APP,"$APP.appointment_id","=","$self.appointment_id")
		...
		if(!empty($res))
		{
			foreach ($res as $key => $value) {
				...
				// $EPR_GROSS_PROFIT 	= _FormatNumberV2($GROSS_PROFIT + $EPR_EARNING);
	...
	public static function readytodispachByMRF($request)
	{
		...
		if (!empty($SELECT_RES)) {
			...
			foreach($SELECT_RES as $ROW)
			{
				...
				$arrProductSalesPlan 					= self::getProductSalesPlan($ROW->PRODUCT_ID,$ROW->MRF_ID,$ROW->Current_Stock,$StockDate);
	...
	*/
	public static function readytodispachByBaseLocation($request)
	{
		...
		$SELECT_SQL 	= "	SELECT base_location_master.base_location_name as MRF_NAME,
							...
							WHERE stock_ladger.product_type = ".PRODUCT_SALES."
							AND stock_ladger.stock_date = '".$StockDate."'
							AND wm_product_master.product_category != 'Inert'
							$WhereCond
		...
		if (!empty($SELECT_RES)) {
			foreach($SELECT_RES as $SELECT_ROW) {
				$arrProductSalesPlan['SALES_PLAN'][]= array("CLIENT_NAME"=>$SELECT_ROW->CLIENT_NAME,
															"SALES_QTY"=>_FormatNumberV2($SELECT_ROW->SALES_QTY,2),
	...
	*/
	public static function GetHSNWiseSalesDetailsReport($req){
		...
		if(!empty($MRFID)){
			// prd($MRFID);
			$SELECT_RES = WmProductMaster::select("id","title","hsn_code","net_suit_code")->where("status",1)->get()->toArray();
			...
			if(!empty($SERVICE_PRO_SELECT_RES)){
				foreach($SERVICE_PRO_SELECT_RES as $KEY_V => $VA){
					...
					WHERE wm_service_master.invoice_date BETWEEN '".date("Y-m-d",strtotime($startDate))."' AND '".date("Y-m-d",strtotime($endDate))."'
	...
	*/
	...
	{
		return Utilities::DigitalSignature($pdfPath,$fullPath,$signedFileName);
	}

	/*
	Use 	: Send Recycleble Dispatch To EPR By DispatchID
	Author 	: Kalpak Prajapati
	Date 	: 29 Dec,2022
	*/
	public static function SendRecyclebleDispatchToEPRByDispatchID($request)
	{
		$WmDispatchProductTbl 	= new WmDispatchProduct();
		$ProductTbl 			= new WmProductMaster();
		$Vehicle				= new VehicleMaster();
		...
		$RES = $data->where("approval_status",1)->orderBy("id","ASC")->first();
		if(!$RES) {
			return $RAW;
		}
		$RAW = $RES->toArray();
		if(!empty($RAW)) {
			$DISPATCH_IMAGES_ARR = array();
			/** Changed by Kalpak @Since 2023-06-12 Check type from Database instead of Static Product ID Array */
			$FLEXI_CNT = WmDispatchProduct::select("wm_dispatch_product.id")
							->leftjoin("wm_product_master","wm_product_master.id","=","wm_dispatch_product.product_id")
							->whereIn("wm_product_master.epr_product_type",explode(",",EPR_FLEXI_ID))
							->where("wm_dispatch_product.dispatch_id",$RAW['id'])
							->count();
			$RIGID_CNT = WmDispatchProduct::select("wm_dispatch_product.id")
							->leftjoin("wm_product_master","wm_product_master.id","=","wm_dispatch_product.product_id")
							->whereIn("wm_product_master.epr_product_type",explode(",",EPR_RIGID_ID))
							->where("wm_dispatch_product.dispatch_id",$RAW['id'])
							->count();
			/** Changed by Kalpak @Since 2023-06-12 Check type from Database instead of Static Product ID Array */
			/** NOT REQUIRED TO SEND TO EPR */
	...
	/*
	Use 	: Send Recycleble Dispatch To EPR
	Author 	: Kalpak Prajapati
	Date 	: 29 Dec,2022
	*/
	public static function SendRecyclebleDispatchToEPR()
	{
		$self 					= (new static)->getTable();
		$EPRTbl					= new LrEprMappingMaster();
		$EPR 					= $EPRTbl->getTable();
		...
		$startDate 				= EPR_START_DATE." ".GLOBAL_START_TIME;
		$endDate 				= $Date." ".GLOBAL_END_TIME;
		$DispatchIDs 			= defined("RECYCLEBLE_TYPE_DISPATCH_TO_TRANSFER")?RECYCLEBLE_TYPE_DISPATCH_TO_TRANSFER:array(0);
		$EPR_PRODUCT_ID 		= array_merge(FLEXI_PRODUCT_ARRAY,RIGID_PRODUCT_ARRAY);
		$GET_LAST_DISPATCH_ID 	= self::select(	"$EPR.dispatch_id",
												"$self.id",
												"$self.challan_no",
									...
									->leftjoin($Client->getTable()." AS CLIENT","$self.destination","=","CLIENT.id")
									->where("CLIENT.gstin_no","!=","")
									->where("$self.dispatch_type",RECYCLEBLE_TYPE)
									->whereNotNull("$self.epr_billt_media_id")
									->whereNotNull("CLIENT.gstin_no")
									->whereNotNull("$self.epr_waybridge_slip_id")
									->whereBetween("$self.dispatch_date",[$startDate,$endDate])
		...
		if (!empty($DispatchIDs)) {
			$GET_LAST_DISPATCH_ID->whereIn("$self.id",array_unique($DispatchIDs));
		}
		$EPR_SQL = LiveServices::toSqlWithBinding($GET_LAST_DISPATCH_ID,true);
		echo "\r\n EPR_SQL :: ".$EPR_SQL."\r\n";
		$DISPATCH_ROWS = $GET_LAST_DISPATCH_ID->get();
		if(!empty($DISPATCH_ROWS))
		{
			foreach($DISPATCH_ROWS AS $RAW)
			{
				...
				$request 				= new Request();
				$request->dispatch_id 	= $RAW->id;
				$data 					= self::SendRecyclebleDispatchToEPRByDispatchID($request);
				if(!empty($data))
				{
					echo "\r\n Dispatch ID ::".$data['id']." -- \r\n";
					$array 				= $data;
					...
					echo "\r\n Dispatch ID ::".$RAW->id." not contains Flexi or Rigid Product Types -- \r\n";
				}
			}
		}
	}
	...
	/*
	Use     	: check validation for generate invoice againts the delivery challan
	Develop 	: Axay Shah
	Date 		: 09 Jan,2023
	*/
	public static function validateToGenerateInvoiceFromDeliveryChallan($request)
	{
		$data 			= array();
		$DispatchID 	= (isset($request->dispatch_id) && !empty($request->dispatch_id)) ? $request->dispatch_id : array();
		if(!empty($DispatchID)){
			$DispatchData 				= WmDispatch::GetById($DispatchID[0]);
			$products 					= WmDispatchProduct::GetProductListByDispatchIDS($DispatchID);
			$data 						= $DispatchData;
			$data['product_data'] 		= $products;
			$data['dispatch_ids_data'] 	= $DispatchID;
			$data['tare_weight'] 		= WmDispatch::whereIn("id",$DispatchID)->sum("tare_weight");
			$data['gross_weight'] 		= WmDispatch::whereIn("id",$DispatchID)->sum("gross_weight");
		}
		return $data;
	}
	/*
	...
	Date 		: 09 Jan,2023
	*/
	public static function EprPendingReport($request)
	{
		$base_location_Ids 	= (isset($request->base_location_id) && !empty($request->base_location_id)) ? $request->base_location_id : array() ;
		if(empty($base_location_Ids)){
			$cityId         = (isset($request->base_location_id) && !empty($request->base_location_id)) ? GetBaseLocationCity($request->base_location_id) : GetBaseLocationCity(Auth()->user()->base_location);
		...
		$DELIVERY_CHALLAN_REPORT_SQL = "SELECT
				...
				(
					CASE
						...
						WHEN wm_dispatch.approval_status = 1 THEN 'Approved'
						WHEN wm_dispatch.approval_status = 2 THEN 'Rejected'
						WHEN wm_dispatch.approval_status = 4 THEN 'Delivery Challan'
					END
				) AS Dispatch_Status,
				wm_dispatch.tcs_amount,
				wm_dispatch.tcs_rate,
				...
				LEFT JOIN `appoinment` ON `wm_dispatch`.`appointment_id` = `appoinment`.`appointment_id`
				LEFT JOIN `customer_master` AS `CM` ON `appoinment`.`customer_id` = `CM`.`customer_id`
				LEFT JOIN lr_epr_mapping_master AS EPR ON wm_dispatch.id = EPR.dispatch_id";

				$DISPATCH_WHERE .= " AND wm_dispatch.dispatch_type=".NON_RECYCLEBLE_TYPE." AND wm_client_master.gstin_no != '' AND wm_dispatch.parent_dispatch_id = 0 AND wm_dispatch.epr_billt_media_id IS NOT NULL AND wm_dispatch.epr_waybridge_slip_id IS NOT NULL";
	...
	*/
	public static function CalculateInvoiceAmount($DispatchObj)
	{
		...
		if($DISPATCH_RENT > 0) {
			$DISPATCH_RENT 	= _FormatNumberV2($DispatchObj->rent_amount);
			$RENT_GST_AMT 	= (($DISPATCH_RENT * $MAX_GST_PERCENT) / 100);
		}
		$TOTAL_RENT_AMT 	= (float)$DISPATCH_RENT + (float)$RENT_GST_AMT;
		$TotalInvoiceAmount += $TOTAL_RENT_AMT;
		$TotalInvoiceAmount -= (!empty($DispatchObj->discount_amt)) ? _FormatNumberV2($DispatchObj->discount_amt) : 0;
		/** CALCULATE FREIGHT GST AMOUNT */
	...
	* @return array $arrDispatch
	* @author Kalpak Prajapati
	* @since 2023-04-18
	* @access public
	* @uses method used to Get List of Dispatches for Purchase Order By BAMS
	*/
	// public static function getDispatchByPurchaseOrder($Request)
	// {
	// 	$arrDispatch 	= array();
	// 	$CURRENTTIME 	= DATE("Y-m-d H:i:s");
	// 	$BAMS_PO_ID 	= (isset($Request->po_id) && !empty($Request->po_id)) ? $Request->po_id : 0;
	// 	$DISPATCH_IDS 	= (isset($Request->dispatch_ids) && !empty($Request->dispatch_ids)) ? $Request->dispatch_ids : array();
	// 	$WhereCond 		= "";
	// 	if (!empty($DISPATCH_IDS)) {
	// 		$DISPATCH_ID = (is_array($DISPATCH_IDS)?$DISPATCH_IDS:explode(",",$DISPATCH_IDS));
	// 		$DISPATCH_ID = implode(",",$DISPATCH_ID);
	// 		$WhereCond .= " AND wm_dispatch.id NOT IN (".$DISPATCH_ID.") ";
	// 	}
	// 	// AND wm_dispatch.dispatch_date BETWEEN transporter_po_details_master.created_at AND '".$CURRENTTIME."'
	// 	$SELECT_SQL 	= "	SELECT wm_dispatch.id AS Dispatch_ID,
	// 						transporter_po_details_master.vehicle_cost_type,
	// 						wm_department.department_name as Bill_From_MRF,
	// 						wm_dispatch.challan_no AS Invoice_No,
	// 						DATE_FORMAT(wm_dispatch.dispatch_date,'%d-%m-%Y') AS Dispatch_Date,
	// 						vehicle_master.vehicle_number as Vehicle_Number,
	// 						wm_dispatch.driver_name as Driver_Name,
	// 						wm_dispatch.driver_mob_no as Driver_Mobile,
	// 						Origin_City.city as Source_City,
	// 						Destination_City.city as Destination_City,
	// 						IF(wm_dispatch.dispatch_type = ".RECYCLEBLE_TYPE.",'Recyclable','Non Recyclable') AS Dispatch_Type,
	// 						wm_dispatch.eway_bill_no as EWayBill_No,
	// 						wm_dispatch.epr_billt_no as BillT_No,
	// 						CASE WHEN 1=1 THEN (
	...
	// 						FROM wm_dispatch
	// 						LEFT JOIN location_master as Origin_City ON Origin_City.location_id = wm_dispatch.origin_city
	// 						LEFT JOIN location_master as Destination_City ON Destination_City.location_id = wm_dispatch.destination_city
	// 						LEFT JOIN vehicle_master ON wm_dispatch.vehicle_id = vehicle_master.vehicle_id
	// 						LEFT JOIN transporter_details_master ON transporter_details_master.dispatch_id = wm_dispatch.id
	...
	// 						AND wm_dispatch.is_from_delivery_challan = 0
	// 						AND transporter_po_details_master.po_id = ".$BAMS_PO_ID."

	// 						$WhereCond
	// 						ORDER BY wm_dispatch.id";
	// 	$SELECT_RES 	= \DB::select($SELECT_SQL);
	// 	\Log::info($SELECT_SQL);
	// 	if (!empty($SELECT_RES)) {
	// 		foreach ($SELECT_RES as $SELECT_ROW) {
	// 			$arrDispatch[] 	= array(
	// 									"vehicle_cost_type"=>$SELECT_ROW->vehicle_cost_type,
	// 									"Dispatch_ID"=>$SELECT_ROW->Dispatch_ID,
	// 									"Bill_From_MRF"=>$SELECT_ROW->Bill_From_MRF,
	// 									"Invoice_No"=>$SELECT_ROW->Invoice_No,
	// 									"Dispatch_Date"=>$SELECT_ROW->Dispatch_Date,
	// 									"Vehicle_Number"=>$SELECT_ROW->Vehicle_Number,
	// 									"Driver_Name"=>$SELECT_ROW->Driver_Name,
	// 									"Driver_Mobile"=>$SELECT_ROW->Driver_Mobile,
	// 									"Source_City"=>$SELECT_ROW->Source_City,
	// 									"Destination_City"=>$SELECT_ROW->Destination_City,
	// 									"Dispatch_Type"=>$SELECT_ROW->Dispatch_Type,
	// 									"EWayBill_No"=>$SELECT_ROW->EWayBill_No,
	// 									"BillT_No"=>$SELECT_ROW->BillT_No,
	// 									"Dispatch_Qty"=>$SELECT_ROW->Dispatch_Qty,
	// 									"Trip_Cost"=>$SELECT_ROW->Trip_Cost,
	// 									"Demurrage_Cost"=>$SELECT_ROW->Demurrage_Cost);
	// 		}
	// 	}
	// 	return $arrDispatch;
	// }
	/**
	* Use  		: Get PO Used details for BAMS
	* Author 	: Axay Shah
	* Date 		: 07-June-2023
	*/
	public static function getDispatchByPurchaseOrder($request)
	{
		...
		$BAMS_PO_ID 	= (isset($request->po_id) && !empty($request->po_id)) ? $request->po_id : 0;
		$DISPATCH_IDS 	= (isset($request->dispatch_ids) && !empty($request->dispatch_ids)) ? $request->dispatch_ids : array();
		$PO_FOR 		= TransporterPoDetailsMaster::where("po_id",$BAMS_PO_ID)->value("po_for");
		if(!empty($PO_FOR)){
			switch($PO_FOR){
				case PARA_PO_FOR_TRANSFER:
					// FOR TRANSFER
					$arrDispatch = WmTransferMaster::getTransferDataByPO($BAMS_PO_ID,$DISPATCH_IDS);
					break;
				case PARA_PO_FOR_COLLECTION:
					// FOR COLLECTION
					$arrDispatch = Appoinment::getCollectionPOData($BAMS_PO_ID,$DISPATCH_IDS);
					break;

				case PARA_PO_FOR_SALES:
					// FOR SALES
					$arrDispatch = self::getDispatchDataByPO($BAMS_PO_ID,$DISPATCH_IDS);
					break;
				case PARA_PO_FOR_JOBWORK:
					// FOR JOBWORK
					$arrDispatch = JobWorkMaster::getJobWorkDataByPO($BAMS_PO_ID,$DISPATCH_IDS);
					break;
			}

			if(!empty($arrDispatch)){
				foreach ($arrDispatch as $SELECT_ROW) {
					$result[] 	= array(
											"Dispatch_ID"=>$SELECT_ROW->Dispatch_ID,
											"vehicle_cost_type"=>$SELECT_ROW->vehicle_cost_type,
											"Bill_From_MRF"=>$SELECT_ROW->Bill_From_MRF,
	...
	/**
	* Use  		: Get Dispatch Data By PO number
	* Author 	: Axay Shah
	* Date 		: 07-June-2023
	*/
	public static function getDispatchDataByPO($BAMS_PO_ID,$DISPATCH_IDS){
		$WhereCond = "";
		if (!empty($DISPATCH_IDS)) {
			$DISPATCH_ID = (is_array($DISPATCH_IDS)?$DISPATCH_IDS:explode(",",$DISPATCH_IDS));
			$DISPATCH_ID = implode(",",$DISPATCH_ID);
			...
			$SELECT_SQL 	= "	SELECT wm_dispatch.id AS Dispatch_ID,
							...
							AND transporter_po_details_master.po_id = ".$BAMS_PO_ID."
							$WhereCond
							GROUP BY wm_dispatch.id ORDER BY wm_dispatch.id";
		$SELECT_RES 	= \DB::select($SELECT_SQL);
		return $SELECT_RES;
	}
	/**
	* Use  		: Download Dispatch Documents
	* Author 	: Axay Shah
	* Date 		: 07-June-2023
	*/
	public static function DownloadDispatchDocument(){
		return false;
		$dispatch_ids 	= array(40494);
		$media_data 	= WmDispatchMediaMaster::whereIn("dispatch_id",$dispatch_ids)->whereIn("media_type",array(PARA_WAYBRIDGE,PARA_EWAY_BILL))->get();
		if(!empty($media_data)){
			foreach($media_data as $key =>  $value){
				$url 	= url('/')."/".$value->image_path."/".$value->image_name;
				$path  	= public_path("/")."VIJENDRA/AUDIT/".$value->dispatch_id;
				if(!is_dir($path)) {
					mkdir($path,0777,true);
				}
				file_put_contents($path."/".$value->image_name, file_get_contents($url));
				$PDF_NAME 			= $path."/".$value->dispatch_id.".pdf";
				$InvoiceId 			= WmInvoices::where("dispatch_id",$value->dispatch_id)->value("id");
				$data       		= WmInvoices::GetById($InvoiceId);
	...
	/*
	Use 	: Send Email to Cement plant client when dispatch is approved daily  basis
	Author 	: Axay Shah
	Date 	: 03 Auguest,2023
	*/
	public static function SendDispatchDetailEmailToClient()
	{
		$WDESL = new WmDispatchEmailSendLog();
		if(strtotime("10:30:00") < strtotime(date("H:i:s"))) {
			$start_time	= date("Y-m-d")." 10:00:00";
			$today  	= date("Y-m-d")." 17:00:00";
		} else {
			$start_time = date('Y-m-d', strtotime("-1 days"))." 17:01:00";
			$today 		= date("Y-m-d")." 10:00:00";
		}
		$clientData = WmClientMaster::select("id","email_for_notification",'client_name')
						->where("email_notification_enable",1)
						->where("status","A")
						->whereNotNull("email_for_notification")
						->get()
						->toArray();
		if(!empty($clientData))
		{
			foreach($clientData as $raw => $value)
			{
				$client_id 		= $value['id'];
				$client_name 	= $value['client_name'];
				$ToEmail 		= (isset($value['email_for_notification']) && !empty($value['email_for_notification']))?explode(",",$value['email_for_notification']):array();
				$dispatch_data 	= WmDispatch::select(DB::raw("WPM.title as product_name"),
													DB::raw("(CASE WHEN WPM.is_afr = 1 THEN 'Shredded'
									...
									->leftjoin($WDESL->getTable()." as WDESL","wm_dispatch.id","=","WDESL.dispatch_id")
									...
									->whereBetween("wm_dispatch.challan_date",array($start_time,$today))
									->where("wm_dispatch.client_master_id",$client_id)
									->where("wm_dispatch.is_from_delivery_challan",0)
				...
				if(!empty($dispatch_data))
				{
					$Subject 	= "Dispatch details";
					$Title 		= "Dispatch details";
					$FromEmail 	= array("Name" => EMAIL_FROM_NAME,"Email" => EMAIL_FROM_ID); //constants defined in whitelable.php
					$sendEmail  = Mail::send("email-template.SendDispatchDetailEmailToClient",array("result"=>$dispatch_data,"HeaderTitle"=>$Subject,"Title"=>$Title,"client_name"=>$client_name), function ($message) use ($ToEmail,$FromEmail,$Subject,$Title) {
							$message->from($FromEmail['Email'], $FromEmail['Name']);
							$message->to($ToEmail);
							$message->bcc(array("laukik.bajpai@nepra.co.in"));
							$message->subject($Subject);
					});
					foreach($dispatch_data as $dkey=> $dvalue){
						WmDispatchEmailSendLog::StoreEmailSentLogForDispatch($value['email_for_notification'],$dvalue['dispatch_id'],$client_id);
					}
				}
			}
		}
	...
	/*
	Use 	: List Dispatch Invoice Which are pending to send in EPR report and widget
	Author 	: Hardesh gupta
	Date 	: 24 Auguest,2023
	*/
	public static function PendingToSendEPRInvoiceFromLR($request)
	{
		$start_date 		= (isset($request->start_date) && !empty($request->start_date)) ? Carbon::parse($request->start_date)->format('Y-m-d')  : date("Y-m-01");
		$end_date 			= (isset($request->end_date) && !empty($request->end_date)) ? Carbon::parse($request->end_date)->format('Y-m-d') : date("Y-m-t");
		$STARTDATE 			= $start_date." ".GLOBAL_START_TIME;
		$ENDDATE 			= $end_date." ".GLOBAL_END_TIME;
		$mrf_id 			= (isset($request->mrf) && !empty($request->mrf)) ? $request->mrf : "";
		$client_name 		= (isset($request->client_name) && !empty($request->client_name)) ? $request->client_name : "";
		$client_id 			= (isset($request->client_id) && !empty($request->client_id)) ? $request->client_id : "";
		...
		$challan_no 		= (isset($request->challan_no) && !empty($request->challan_no)) ? $request->challan_no : 0;
		$dispatch_id 		= (isset($request->dispatch_id) && !empty($request->dispatch_id)) ? $request->dispatch_id : 0;
		$is_widget 			= (isset($request->is_widget) && !empty($request->is_widget)) ? $request->is_widget : 0;
		$where 				= "";
		if(!empty($client_name)) {
			$where .= " AND CLIENT.client_name like '%$client_name%'";
		}
		if(!empty($product)) {
			$where .= " AND WDP.product_id = $product";
		}
		if($mrf_id > 0) {
			$where .= " AND WD.bill_from_mrf_id = $mrf_id";
		}
		if(!empty($vehicle_number)) {
			$where .= " AND V.vehicle_number = '$vehicle_number'";
		}
		if(!empty($approval_status)) {
			$where .= " AND WD.approval_status = $approval_status";
		}
		if($mrf_id > 0) {
			$where .= " AND WD.bill_from_mrf_id = $mrf_id";
		}
		if($challan_no > 0) {
			$where .= " AND WD.challan_no = $challan_no";
		}
		if($dispatch_id > 0) {
			$where .= " AND WD.id = $dispatch_id";
		}
		if(!empty($client_id)) {
			$where .= " AND WD.client_master_id = $client_id";
		}
		if(!empty($STARTDATE) && !empty($ENDDATE)) {
			$where .= " AND WD.dispatch_date BETWEEN '".$STARTDATE."' AND '".$ENDDATE."'";
		}

		$SQL 	= "	SELECT WD.id,WD.challan_no,SUM(WDP.quantity) as quantity,WD.dispatch_date, WD.origin,WD.origin_city,
					ORI.city as origin_city,WD.origin_state_code, WD.destination,WD.bill_from_mrf_id,WD.epr_credit,
					WD.destination_city,WD.destination_state_code,WD.shipping_address,WD.shipping_city,WD.shipping_state,
					DEST.department_name as bill_from,WD.client_master_id,WD.epr_billt_media_id,WD.epr_waybridge_slip_id,
					CLIENT.client_name,WD.approval_status,V.vehicle_number,WDP.product_id,WPM.title as product_name,
					CASE
						WHEN WD.approval_status = 1 Then 'Approved'
						WHEN WD.approval_status = 4 Then 'Approved DC'
					END AS approval_status_name,
					CASE WHEN 1=1 THEN
					(
						SELECT epr_credit
						FROM wm_client_master_po_details
						WHERE wm_client_master_po_details.wm_client_id = WD.client_master_id
						AND wm_client_master_po_details.wm_product_id = WDP.product_id
						AND wm_client_master_po_details.status = 1
						AND wm_client_master_po_details.end_date BETWEEN '$STARTDATE' AND '$ENDDATE'
						ORDER BY id DESC
						LIMIT 1
					) END AS EPR_CREDIT_STATUS,
					IF (WD.epr_billt_media_id = 0 OR WD.epr_billt_media_id IS NULL,'".trans("message.EPR_BILTY_PENDING_REASON")."','".trans("message.EPR_WAYBRIDGE_PENDING_REASON")."') AS reason
					FROM wm_dispatch_product as WDP
					INNER JOIN wm_dispatch WD on WD.id = WDP.dispatch_id
					INNER JOIN vehicle_master V on WD.vehicle_id = V.vehicle_id
					INNER JOIN wm_department DEST on WD.bill_from_mrf_id = DEST.id
					INNER JOIN wm_client_master CLIENT on WD.client_master_id = CLIENT.id
					INNER JOIN wm_product_master WPM on WDP.product_id = WPM.id
					LEFT JOIN location_master ORI on WD.origin_city = ORI.location_id
					LEFT JOIN lr_epr_mapping_master as LEMM on WD.id = LEMM.dispatch_id
					LEFT JOIN wm_client_master_po_details as CLPO on CLPO.wm_client_id = WD.client_master_id AND CLPO.wm_product_id = WDP.product_id AND CLPO.status = 1
					WHERE WD.approval_status IN (1,4)
					AND WD.is_from_delivery_challan IN (0)
					AND WD.parent_dispatch_id = 0
					AND WD.dispatch_type = ".NON_RECYCLEBLE_TYPE."
					AND (CLIENT.gstin_no != '' OR CLIENT.gstin_no IS NOT NULL)
					AND ((WD.epr_billt_media_id = 0 OR WD.epr_billt_media_id IS NULL) OR (WD.epr_waybridge_slip_id = 0 OR WD.epr_waybridge_slip_id IS NULL))
					AND (LEMM.dispatch_id IS NULL OR LEMM.process = 0)
					$where";
		if($is_widget == 1) {
			$SQL .= " GROUP BY WD.client_master_id,WD.bill_from_mrf_id,WDP.product_id";
		} else {
			$SQL .= " GROUP BY WD.id,WDP.product_id";
		}
		$SQL .= " HAVING (EPR_CREDIT_STATUS = 1 OR EPR_CREDIT_STATUS IS NULL OR WD.epr_credit = 1)";
		$result = DB::select($SQL);
		return $result;
	}
	...
	*/
	public static function GetClientPOEPRCreditData($request)
	{
		...
		$product 			= (isset($request->product) && !empty($request->product)) ? $request->product : array();
		$direct_dispatch 	= (isset($request->direct_dispatch) && !empty($request->direct_dispatch)) ? $request->direct_dispatch : 0;
		$customer_id 		= (isset($request->customer_id) && !empty($request->customer_id)) ? $request->customer_id : 0;
		$result 			= "";
		$today_date 		= date('Y-m-d');
		if(($direct_dispatch == 1)) {
			$result = ViewCustomerMaster::select("epr_credit")->where('customer_id',$customer_id)->first();
		} else if(($direct_dispatch == 0)) {
			$sqlQry = WmClientPurchaseOrders::select("epr_credit")->where('wm_client_id',$client_id)->where('mrf_id',$mrf_id)->where('status',1)->whereDate('created_at',$today_date);
			if(!empty($product)) {
				$sqlQry->whereIn('wm_product_id',$product);
			}
			$sqlQry->orderBy('id', 'DESC');
			$result = $sqlQry->first();
		}
		$result['epr_credit'] = (empty($result))?"0":$result->epr_credit;
		return $result;
	}

	/*
	Use     : Reduction Data for Eway Bill
	Author  : Axay Shah
	Date 	: 11 Octomber 2023
	*/
	public static function DispatchDocumentReduction(){
		$DISPATCH_DATA = self::select("wm_dispatch.*")
		->leftjoin("lr_epr_document_strick_out_master","wm_dispatch.id","=","lr_epr_document_strick_out_master.dispatch_id")
		->where("wm_dispatch.epr_ewaybill_media_id",">",0)
		->whereNull('lr_epr_document_strick_out_master.dispatch_id')
		->where("wm_dispatch.epr_credit",1)
		->first();

		if(!empty($DISPATCH_DATA)){
			$DISPATCH_ID = $DISPATCH_DATA->id;
			$media = WmDispatchMediaMaster::where("dispatch_id",$DISPATCH_ID)->where("media_type",PARA_EWAY_BILL)->first();
			if(!empty($media)){
				$path 		= public_path("/")."/".$media->image_path.$media->image_name;
				$file_name 	= "eway_reduction_".$media->image_name;
				$mimeType 	= mime_content_type($path);

				self::CallInvoiceReductionAPI($DISPATCH_ID,PARA_EWAY_BILL,$path,$file_name,$mimeType);
			}
		}
	}
	/*
	...
	Date 	: 11 Octomber 2023
	*/
	public static function CallInvoiceReductionAPI($DispatchID,$documentType="",$target="",$filename="",$mimeType="")
	{
		$url 	  	= REDUCTION_AWS_URL;
		$cFile 		= curl_file_create($target);
		$csv_file 	= array('file'=> $cFile); // Parameter to be sent
		$headers = array(
			'Content-type: multipart/form-data'
		);
		$csv_file 	= new \CURLFile($target,$mimeType,$filename);
		$post_data 	= array(
			  "uploaded_file" => $csv_file);
		$insertID = \DB::table("lr_epr_document_strick_out_master")
		->insertGetId(array(
		...
		if(isset($res['status_code']) && $res['status_code'] == SUCCESS){
			$partialPath 	= "images/company/reduction/";
			$imagePath 		= public_path("/")."/images/company/reduction/";
			if(!is_dir($imagePath)) {
				mkdir($imagePath,0777,true);
			}
			$encoded_document 	= $res['uploaded_file'];
			$newFileName 		= $imagePath."/".$filename;
			$file 				= fopen($newFileName, "w");
			fwrite($file, base64_decode($encoded_document));
			fclose($file);
			$media_id = WmDispatchMediaMaster::AddDispatchMedia($DispatchID,$filename,$filename,$partialPath,PARA_STRICK_OUT_EWAY_BILL);
				\DB::table("lr_epr_document_strick_out_master")->where("id",$insertID)
				->update(array("response_data"	=>$response,"updated_at" =>date("Y-m-d H:i:s"),"status"=>1));
				WmDispatch::where("id",$DispatchID)->update(array("strick_out_eway_bill_id" => $media_id));
		}else{
				\DB::table("lr_epr_document_strick_out_master")->where("id",$insertID)
				->update(array("response_data"	=>$response,"updated_at"	=>date("Y-m-d H:i:s"),"status"=>3));
		}
	...
	/*
	Use     : Send Mail For Pending Einvoice 
	Author  : Hardyesh Gupta
	Date    : 13 Nov,2023
	*/
	public static function SendMailPendingEInvoice($request = ""){
		$date 		= date("Y-m-d",strtotime("-1 days"));

		$start_date = $date." ".GLOBAL_START_TIME;
		$end_date 	= $date." ".GLOBAL_END_TIME;
		$SelectSql	= " (
							SELECT
							wm_dispatch.id AS id,
							wm_dispatch.approval_status As approval_status,
							wm_dispatch.acknowledgement_no As ack_no,
							wm_dispatch.master_dept_id AS mrf_id,
							wm_dispatch.challan_no AS serial_no,
							wm_dispatch.dispatchplan_id AS bill_from_mrf_id,
							SUM(wm_dispatch_product.gst_amount) AS gst_amount,
							CMT.client_name as client_name,
							DPT.department_name as department_name,
							wm_dispatch.created_by AS created_by,
							CONCAT(U1.firstname,' ',U1.lastname) as created_by_name,
							DATE_FORMAT(wm_dispatch.dispatch_date,'%d-%m-%Y') AS created_date,
							'DISPATCH' AS trn_type
							FROM `wm_dispatch`
							JOIN `wm_dispatch_product` ON `wm_dispatch`.`id` = `wm_dispatch_product`.`dispatch_id`
							JOIN adminuser U1 ON wm_dispatch.created_by = U1.adminuserid
							JOIN wm_client_master CMT ON wm_dispatch.client_master_id = CMT.id
							JOIN wm_department DPT on wm_dispatch.bill_from_mrf_id = DPT.id
							WHERE wm_dispatch.acknowledgement_date IS NULL
							AND wm_dispatch.challan_date BETWEEN '$start_date'  AND '$end_date'
							AND CMT.email != ''
							AND wm_dispatch.approval_status = 1 
							AND wm_dispatch_product.gst_amount > 0 
						...
						(
							SELECT
							wm_invoices_credit_debit_notes.id AS id,
							wm_invoices_credit_debit_notes.status As approval_status,
							wm_invoices_credit_debit_notes.ack_no As ack_no,
							wm_invoices_credit_debit_notes.mrf_id AS mrf_id,
							wm_invoices_credit_debit_notes.serial_no AS serial_no,
							wm_invoices_credit_debit_notes.bill_from_mrf_id AS bill_from_mrf_id,
							wm_invoices_credit_debit_notes_details.gst_amount AS gst_amount,
							CMT.client_name as client_name,
							DPT.department_name as department_name,
							wm_invoices_credit_debit_notes.created_by AS created_by,
							CONCAT(U1.firstname,' ',U1.lastname) as created_by_name,
							DATE_FORMAT(wm_invoices_credit_debit_notes.change_date,'%Y-%m-%d') AS created_date,
							'DISPATCH CREDIT DEBIT NOTE' AS trn_type
							FROM `wm_invoices_credit_debit_notes`
							LEFT JOIN `wm_invoices_credit_debit_notes_details` ON `wm_invoices_credit_debit_notes`.`id` = `wm_invoices_credit_debit_notes_details`.`cd_notes_id`
							LEFT JOIN adminuser U1 ON wm_invoices_credit_debit_notes.created_by = U1.adminuserid
							LEFT JOIN wm_dispatch DISPATCH ON wm_invoices_credit_debit_notes.dispatch_id = DISPATCH.id
							LEFT JOIN wm_client_master CMT ON DISPATCH.destination = CMT.id
							LEFT JOIN wm_department DPT on wm_invoices_credit_debit_notes.mrf_id = DPT.id
							WHERE wm_invoices_credit_debit_notes.ack_date IS NULL 
							AND CMT.email != ''
							AND wm_invoices_credit_debit_notes.status = 1 
							AND wm_invoices_credit_debit_notes_details.gst_amount > 0 
							AND wm_invoices_credit_debit_notes.change_date = '$date'
							group by wm_invoices_credit_debit_notes.id
							
						)
						UNION ALL
						(
							SELECT
							wm_service_master.id AS id,
							wm_service_master.approval_status As approval_status,
							wm_service_master.ack_no As ack_no,
							wm_service_master.mrf_id AS mrf_id,
							wm_service_master.serial_no AS serial_no,
							wm_service_master.mrf_id AS bill_from_mrf_id,
							wm_service_product_mapping.gst_amt AS gst_amount,
							CMT.client_name as client_name,
							DPT.department_name as department_name,
							wm_service_master.created_by AS created_by,
							CONCAT(U1.firstname,' ',U1.lastname) as created_by_name,
							DATE_FORMAT(wm_service_master.dated,'%Y-%m-%d') AS created_date,
							'SERVICE' AS trn_type
							FROM `wm_service_master`
							LEFT JOIN `wm_service_product_mapping` ON `wm_service_master`.`id` = `wm_service_product_mapping`.`service_id`
							LEFT JOIN adminuser U1 ON wm_service_master.created_by = U1.adminuserid
							LEFT JOIN wm_client_master CMT ON wm_service_master.client_id = CMT.id
							LEFT JOIN wm_department DPT on wm_service_master.mrf_id = DPT.id
							WHERE wm_service_master.ack_date IS NULL 
							AND wm_service_master.invoice_date = '$date'
							AND CMT.email != ''
							AND wm_service_master.approval_status = 1 
							AND wm_service_product_mapping.gst_amt > 0
							group by wm_service_master.id
						)";
						
		$resultData  = DB::select($SelectSql);

		if(!empty($resultData)){
				$ToEmail				= EMAIL_PENDING_EINVOICE;
				$FromEmail 				= array ("Name" => "Nepra Resource Ltd.","Email" => "donot-reply@nepra.co.in");
				$Subject 				= "Pending EInvoice Details";
				$Message        		= "Hello All";
				$EmailContent			= array();
				$EmailContent['ROWS']	= $resultData;
				$EmailContent['Title']	= "Pending Invoice Detail";
				$sendEmail 				= Mail::send("email-template.PendingEinvoiceSendMail",$EmailContent, function ($message) use ($ToEmail,$FromEmail,$Subject){
										$message->from($FromEmail['Email'], $FromEmail['Name']);
										// $message->to(explode(",",$ToEmail));
										$message->to($ToEmail);
										// $message->bcc(explode(",",$BccEMail));
										// $message->bcc(array("axay.shah@nepra.co.in"));
										$message->subject($Subject);
										});	
			}
	}
